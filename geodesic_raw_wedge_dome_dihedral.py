...
center-X wheel zoom, max-zoom arrow-key pan behavior, and two selectable seam construction systems: raw-wedge/trapezoid-key or shaved-face/flat-key.
...
# ============================================================================
# THE JIG, OPERATED ONE STEP AT A TIME.
#
# The fixture used to appear all at once, fully built and fully loaded, which made it
# impossible to see what any part of it was for.  These are the stages of actually
# using it, in the order a builder does them.  Each entry is
# (slug, headline, what you are looking at, what it is physically enforcing).
JIG_STAGES: tuple[tuple[str, str, str, str], ...] = (
    (
        "bench",
        "1. THE BARE BENCH",
        "One flat sheet on sawhorses, and nothing else. This board is the only "
        "thing in the whole build that has to be true.",
        "Flatness. Every angle below is measured from this one surface, so the "
        "panel cannot wind or twist as long as the board does not.",
    ),
    (
        "triangle",
        "2. THE TRIANGLE, DRAWN ONCE",
        "The panel's mathematical triangle, snapped onto the board. It is drawn "
        "one time, from the geodesic solution, and never measured again.",
        "Repeatability. Forty panels are laid out from three lines that were "
        "struck once, instead of from forty tape measurements.",
    ),
    (
        "rails",
        "3. AXIS RAILS",
        "Three low cleats, one per member, each set parallel to that member's "
        "point-axis and just to the seam side of it.",
        "Position across the panel. Push the stick against its rail and the "
        "sector's point line sits exactly where the solution says. The rail "
        "references the sawn point, never the bark, so a lumpy log does not move "
        "the joint.",
    ),
    (
        "fences",
        "4. RED / GREEN ORIENTATION FENCES",
        "Two short cradle plates per member, standing against the two sawn radial "
        "faces of the wedge.",
        "Rotation about the stick's own axis. A 45-degree sector dropped in "
        "rotated 90 or 180 degrees still LOOKS fine; these plates make it not "
        "fit. Red side to red plate, green side to green plate, or the stick does "
        "not seat.",
    ),
    (
        "guides",
        "5. THE TWO CUT PLANES",
        "The magenta plate at each head end is the flush-cut fence. The cyan plate "
        "is the receiving face the next stick's butt will land on.",
        "The saw. A circular saw or a handsaw run flat against the magenta face "
        "cuts the head to the exact plane of the triangle edge, without anyone "
        "setting a bevel or a mitre angle.",
    ),
    (
        "buttcut",
        "6. THE BUTT CUT, MADE OFF THE JIG",
        "One stick, still full length at both ends, laid up to the cyan plate. The "
        "amber tail past that plate is the stock the butt cut removes.",
        "Batching. This is the ONLY cut made before assembly, and every stick of "
        "the same family takes the same one, so it can be run through a saw bench "
        "in a batch rather than fitted one at a time.",
    ),
    (
        "member1",
        "7. FIRST STICK ON THE BENCH",
        "The same stick, butt now cut, dropped against its rail and its two "
        "fences. The head end is still long and hangs off the board.",
        "Two ends, two different operations. The butt arrives finished; the head "
        "arrives long on purpose and is not touched yet.",
    ),
    (
        "member2",
        "8. SECOND STICK, BUTT ONTO SIDE",
        "The second stick lands so that the FIRST stick's butt bears on its "
        "radial face -- on the side of it, not on its end.",
        "End-to-side. Nothing is mitred to meet at a point; the joint is a flat "
        "sawn face pressed onto a flat sawn face, which is what a split wedge "
        "already has without any further machining.",
    ),
    (
        "member3",
        "9. THE LAST STICK SLIDES IN",
        "The third stick is captured at BOTH ends once the other two are down, so "
        "it cannot drop in from above. It is shown backed off along its own axis, "
        "ready to slide home into the pocket.",
        "The chain is cyclic. This is the one awkward move in the whole method, "
        "and it is why the head end is left long: the extra length is what gives "
        "the last stick somewhere to come from.",
    ),
    (
        "closed",
        "10. PULLED UP TIGHT",
        "Home. Three butts bearing on three sides, the green outlines are the "
        "actual contact faces, and all three heads still hang off long.",
        "Self-locating. With the loop closed the frame holds its own shape and "
        "the jig is only keeping it flat while the heads are trimmed.",
    ),
    (
        "flush",
        "11. FLUSH-CUT THE THREE HEADS",
        "Each head is sawn back to its magenta fence. The orange stock hanging "
        "past the board is what falls off.",
        "This is the whole trick. The head is never measured. It is cut in place, "
        "to the plane the neighbouring member has already established, so error "
        "in stock length or in the butt angle leaves as offcut instead of "
        "accumulating around the triangle.",
    ),
    (
        "panel",
        "12. PANEL OFF THE JIG",
        "A finished triangular frame, its three corners clean inside the "
        "mathematical triangle. The jig is empty and unchanged.",
        "Forty of these come off the same fixture. Nothing on the jig was "
        "adjusted between panels, so panel forty is panel one.",
    ),
)

JIG_STAGE_COUNT = len(JIG_STAGES)
JIG_STAGE_ASSEMBLED = JIG_STAGE_COUNT - 1


def jig_stage_index(slug: str) -> int:
    for i, entry in enumerate(JIG_STAGES):
        if entry[0] == slug:
            return i
    raise ValueError(f"Unknown jig stage: {slug}")
...
class DomeConfig:
    ...
    # SEAM CONSTRUCTION SYSTEM.
    # raw_trapezoid: preserve the raw 1/8-log seam faces and use a truncated wedge/
    #                trapezoid key to absorb the remaining raw-face angle.
    # shaved_flat:   longitudinally shave both seam faces parallel to the dihedral
    #                bisector and install a rectangular/flat key between them.
    seam_join_mode: str = "raw_trapezoid"  # raw_trapezoid | shaved_flat
    trapezoid_nose_depth_in: float = 0.50   # truncates the old triangular key into a trapezoid
    flat_key_width_in: float = 1.00         # width across the seam for shaved_flat mode
    flat_key_depth_in: float = 4.00         # radial key depth from the exterior seam ridge inward
    ...
    # Sacrificial raw-stock allowance beyond the solved pinwheel butt plane.  In jig
    # view the final member remains visible, while this extra tail is rendered separately
    # so it can be aligned by the fixture, scribed/flush-cut at the exact receiver plane,
    # and discarded.  This does NOT alter finished dome geometry.
    jig_butt_hangoff_in: float = 8.0

    # HEAD-END (receiving/through end) OVERFIT.
    #
    # The two ends of a member are made in completely different ways, and the model now
    # says so.  The BUTT end is cut before assembly, off the jig, to the compound angle
    # its receiving neighbour presents.  The HEAD end is NOT.  It is left deliberately
    # long, so that when all three butts are pulled up tight the head overshoots the
    # triangle, and it is then flush-cut in place at the plane the neighbouring member
    # has already established.  This value is how much it overshoots by.
    #
    # Finished dome geometry is unchanged: this is stock that becomes offcut.  Its whole
    # purpose is to absorb stock-length and butt-angle error at the ONE end where error
    # can be thrown away instead of accumulating around the triangle.
    head_overfit_in: float = 6.0

    # Which step of the jig walk-through is on screen. See JIG_STAGES.
    jig_stage: int = JIG_STAGE_ASSEMBLED
    ...
    def validate(self) -> None:
        ...
        if self.seam_join_mode not in {"raw_trapezoid", "shaved_flat"}:
            raise ValueError("seam_join_mode must be raw_trapezoid or shaved_flat")
        if self.trapezoid_nose_depth_in < 0:
            raise ValueError("trapezoid_nose_depth_in must be >= 0")
        if self.flat_key_width_in <= 0 or self.flat_key_depth_in <= 0:
            raise ValueError("flat key width/depth must be positive")
        ...
        if self.jig_butt_hangoff_in < 0:
            raise ValueError("jig_butt_hangoff_in must be >= 0")
        if self.head_overfit_in < 0:
...
def pinwheel_receiver_surface_label(config: DomeConfig) -> str:
    """Human-readable physical surface used as the pinwheel receiving datum.

    RED/GREEN match the colored radial faces in the rendered raw sector.  The special
    POINT_PANEL_OUT state points the sector across the seam, leaving the curved bark/back
    toward the panel interior; that case therefore uses a tangent datum rather than a broad
    flat radial face.
    """
    if config.wedge_orientation in {"point_dome_in", "point_panel_in"}:
        return "GREEN"
    if config.wedge_orientation == "point_dome_out":
        return "RED"
    return "BROWN_TANGENT"
...
def export_bom(model: DomePhysicalModel, path: str | Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for s in model.seams:
            ...
            if cfg.seam_join_mode == "raw_trapezoid":
                fraction = clamp(cfg.trapezoid_nose_depth_in / max(side, 1.0e-9), 0.0, 0.95)
                area = 0.5 * base * height * (1.0 - fraction * fraction)
                spacer_object_type = "rigid_trapezoid_raw_wedge_key"
            else:
                area = cfg.flat_key_width_in * min(cfg.flat_key_depth_in, cfg.trunk_diameter_in * 0.75)
                spacer_object_type = "rigid_flat_key_shaved_seam_faces"
...
def export_cut_schedule(model: DomePhysicalModel, path: str | Path) -> None:
    ...
    Each member receives TWO deliberately different cuts, made at different times and in
    different places, and the schedule now says which is which:
      * BUTT/PINWHEEL CUT -- made BEFORE assembly, off the jig, on a saw bench. Compound
        plane coincident with the next member's interior radial face. Identical for every
        member of the same family, so it batches.
      * RECEIVE/VERTEX CLEAN CUT -- made AFTER assembly, in place, against the jig fence.
        Triangle-envelope plane through the geodesic node. The member arrives at the jig
        carrying ``head_overfit_in`` of extra length past this plane; that extra becomes
        offcut, which is how stock-length and butt-angle error leaves the panel instead of
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow(
            [
                ...
                "mate_member_id", "pinwheel_receiver_surface", "jig_butt_hangoff_in",
                "jig_cut_blank_length_in", "cut_when", "cut_where",
                "head_overfit_in", "arrives_at_jig_length_in",
            ]
        )
        for m in model.members:
            ...
            writer.writerow(
                [
                    ...
                    pinwheel_receiver_surface_label(model.config),
                    f"{model.config.jig_butt_hangoff_in:.6f}",
                    f"{m.physical_stock_length_in + model.config.jig_butt_hangoff_in:.6f}",
                    "AFTER_ASSEMBLY",
                    "IN_PLACE_AGAINST_JIG_HEAD_FENCE",
                    f"{model.config.head_overfit_in:.6f}",
                    f"{m.physical_stock_length_in + model.config.head_overfit_in:.6f}",
                ]
            )
...
def export_panel_jig_svg(model: DomePhysicalModel, panel_index: int, path: str | Path) -> None:
    ...
    parts.append('<text x="12" y="40" font-family="monospace" font-size="12" fill="#aa00aa">MAGENTA GUIDE = CLEAN VERTEX CUT</text>')
    parts.append('<text x="250" y="40" font-family="monospace" font-size="12" fill="#0087a8">CYAN GUIDE = FLUSH BUTT CUT PLANE</text>')
    parts.append(f'<text x="12" y="56" font-family="monospace" font-size="11">WEDGE SIDES: RED / GREEN; PINWHEEL RECEIVER = {html.escape(pinwheel_receiver_surface_label(model.config))}; ORANGE = +{model.config.jig_butt_hangoff_in:.1f}in SACRIFICIAL TAIL</text>')
    ...
    for m in members:
        ...
        tail_world = origin + (m.butt_axis_parameter_in + model.config.jig_butt_hangoff_in) * m.tangent
        tx, ty = pt(*_panel_xy(tail_world, frame))
        parts.append(f'<line x1="{bx:.3f}" y1="{by:.3f}" x2="{tx:.3f}" y2="{ty:.3f}" stroke="#ff8800" stroke-width="4" stroke-opacity="0.55"/>')

        rpoly = [_panel_xy(p, frame) for p in m.receiving_cut_points]
        bpoly = [_panel_xy(p, frame) for p in m.butt_contact_points]
        parts.append(f'<polyline points="{points_string(rpoly + [rpoly[0]])}" fill="none" stroke="#cc00cc" stroke-width="2"/>')
        parts.append(f'<polyline points="{points_string(bpoly + [bpoly[0]])}" fill="none" stroke="#00a8d6" stroke-width="2"/>')
        mx = (ax + bx) * 0.5; my = (ay + by) * 0.5
        label = f'{m.member_id} {m.edge_type} finished={m.physical_stock_length_in:.3f}in jigblank={m.physical_stock_length_in + model.config.jig_butt_hangoff_in:.3f}in'
...
def export_seam_join_schedule(model: DomePhysicalModel, path: str | Path) -> None:
    """Export the two 2V seam families and both join-strategy dimensions."""
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    cfg = model.config
    sector_angle = 360.0 / cfg.radial_splits
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "seam_id", "edge_type", "seam_length_in", "fold_angle_deg",
            "included_face_angle_deg", "raw_sector_angle_deg",
            "45deg_reference_minus_fold_deg", "actual_selected_raw_face_gap_deg",
            "active_join_mode", "raw_trapezoid_outer_width_in",
            "raw_trapezoid_inner_width_in", "flat_key_width_in", "flat_key_depth_in",
        ])
        for seam in model.seams:
            length = float(np.linalg.norm(seam.end - seam.start))
            fraction = clamp(cfg.trapezoid_nose_depth_in / max(seam.contact_depth_in, 1.0e-9), 0.0, 0.95)
            writer.writerow([
                seam.seam_id,
                seam.edge_type,
                f"{length:.6f}",
                f"{seam.fold_angle_deg:.6f}",
                f"{seam.internal_dihedral_deg:.6f}",
                f"{sector_angle:.6f}",
                f"{sector_angle - seam.fold_angle_deg:.6f}",
                f"{seam.raw_gap_angle_deg:.6f}",
                cfg.seam_join_mode,
                f"{seam.spacer_base_width_in * fraction:.6f}",
                f"{seam.spacer_base_width_in:.6f}",
                f"{cfg.flat_key_width_in:.6f}",
                f"{min(cfg.flat_key_depth_in, cfg.trunk_diameter_in * 0.75):.6f}",
            ])

def export_jig_cut_sequence(model: DomePhysicalModel, path: str | Path) -> None:
    """Export the whole per-panel procedure, in the order it is actually carried out.

    Two phases, because the two ends of a member are made in different places.

    PHASE 1 -- BUTT, off the jig.  The uncut terminating member cannot occupy the same
    solid volume as the receiving wood, so the cyan guide plate is used as a RECEIVER-FACE
    PROXY: clamp the raw wedge in its red/green orientation fences, let the sacrificial
    tail pass the proxy plane, scribe/flush-cut, then dry-fit.  Every member of a family
    takes the same cut, so this batches.

    PHASE 2 -- HEAD, on the jig, after the pinwheel closes.  The member arrives carrying
    ``head_overfit_in`` past its finished head plane.  Three butts are pulled up tight,
    the loop closes, and only then is each head sawn against the magenta fence.  Nothing
    is measured in phase 2; the plane is established by the neighbouring member.
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    receiver_surface = pinwheel_receiver_surface_label(model.config)
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "panel_id", "panel_type", "phase", "cut_step", "operation",
            "member_id", "mates_with_member", "receiver_surface", "wedge_orientation",
            "allowance_in", "finished_blank_in", "stock_at_this_step_in",
            "jig_walkthrough_step", "fixture_action",
        ])
        buttcut_step = jig_stage_index("buttcut") + 1
        flush_step = jig_stage_index("flush") + 1
        for face in model.topology.faces:
            members = sorted(
                [m for m in model.members if m.face_index == face.index],
                key=lambda m: m.local_edge_index,
            )
            for step, member in enumerate(members, start=1):
                writer.writerow([
                    f"PANEL_{face.index + 1:03d}", face.face_type,
                    "1_BUTT_OFF_JIG", step, "COMPOUND_BUTT_TO_RECEIVER_INTERIOR_FACE",
                    member.member_id, member.butt_into_member_id, receiver_surface,
                    model.config.wedge_orientation,
                    f"{model.config.jig_butt_hangoff_in:.6f}",
                    f"{member.physical_stock_length_in:.6f}",
                    f"{member.physical_stock_length_in + model.config.jig_butt_hangoff_in:.6f}",
                    f"{buttcut_step}/{JIG_STAGE_COUNT}",
                    "CLAMP_AXIS_AND_RED_GREEN_ROTATION; USE_CYAN_RECEIVER_PROXY; "
                    "FLUSH_CUT_AMBER_TAIL; DRY_FIT_RECEIVER",
                ])
            for step, member in enumerate(members, start=1):
                writer.writerow([
                    f"PANEL_{face.index + 1:03d}", face.face_type,
                    "2_HEAD_ON_JIG", step, "FLUSH_CUT_HEAD_AT_TRIANGLE_ENVELOPE_PLANE",
                    member.member_id, member.receives_member_id, receiver_surface,
                    model.config.wedge_orientation,
                    f"{model.config.head_overfit_in:.6f}",
                    f"{member.physical_stock_length_in:.6f}",
                    f"{member.physical_stock_length_in + model.config.head_overfit_in:.6f}",
                    f"{flush_step}/{JIG_STAGE_COUNT}",
                    "PINWHEEL_CLOSED_AND_CLAMPED; SAW_FLAT_ON_MAGENTA_HEAD_FENCE; "
                    "DISCARD_ORANGE_OFFCUT; NO_MEASUREMENT_TAKEN",
                ])
...
def export_jig_walkthrough(model: DomePhysicalModel, path: str | Path) -> None:
    ...
    The same table the 3-D walk-through steps through, so somebody at the bench has the
    order of operations and what each part of the fixture is for, without the computer.
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["step", "slug", "headline", "what_you_are_looking_at",
                         "what_it_enforces"])
        for index, (slug, headline, looking_at, enforcing) in enumerate(JIG_STAGES, start=1):
            writer.writerow([index, slug, headline, looking_at, enforcing])
...
def export_fabrication_package(model: DomePhysicalModel, directory: str | Path) -> None:
    ...
    for variant_number, (_, panel_indices) in enumerate(sorted(groups.items(), key=lambda item: item[1][0]), start=1):
        ...
        # The finished panel and the loaded bench are different objects, so both are
        # written: one is what comes off the jig, the other is what sits on it while the
        # heads are still long.
        jig = build_fabrication_jig(model, representative)
        loaded = build_fabrication_jig(model, representative, jig_stage_index("closed"))
        export_mesh_obj(jig["jig_fixture"], variant_obj_dir / f"{variant_id}_FIXTURE.obj", variant_id + "_FIXTURE")
        export_mesh_obj(jig["jig_wood"], variant_obj_dir / f"{variant_id}_EXAMPLE_FINISHED_WOOD.obj", variant_id + "_EXAMPLE_FINISHED_WOOD")
        export_mesh_obj(loaded["jig_head_overfit"], variant_obj_dir / f"{variant_id}_HEAD_OVERFIT_OFFCUT.obj", variant_id + "_HEAD_OVERFIT_OFFCUT")
        cutting = build_fabrication_jig(model, representative, jig_stage_index("buttcut"))
        export_mesh_obj(cutting["jig_hangoff"], variant_obj_dir / f"{variant_id}_SACRIFICIAL_BUTT_HANGOFF.obj", variant_id + "_SACRIFICIAL_BUTT_HANGOFF")

    with (directory / "JIG_VARIANTS.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["jig_variant", "wedge_orientation", "representative_panel", "compatible_panels", "panel_count", "pinwheel_receiver_surface", "jig_butt_hangoff_in", "head_overfit_in"])
...
JIG_BASE_COLOR = (0.34, 0.36, 0.40, 0.88)
JIG_RAIL_COLOR = (0.78, 0.80, 0.84, 1.0)
JIG_GUIDE_VERTEX_COLOR = (0.95, 0.12, 0.95, 0.68)
JIG_VERTEX_CUT_LINE_COLOR = (1.00, 0.25, 1.00, 1.0)
# Butt flush-cut plane is cyan so it cannot be confused with the GREEN wedge face.
JIG_GUIDE_BUTT_COLOR = (0.10, 0.82, 1.00, 0.58)
JIG_RED_FACE_FENCE_COLOR = (0.98, 0.16, 0.16, 0.72)
JIG_GREEN_FACE_FENCE_COLOR = (0.16, 0.96, 0.26, 0.72)
JIG_HANGOFF_COLOR = (1.00, 0.55, 0.08, 0.36)
# Head-end overfit is the stock that is flush-cut away in place. Warmer and more solid
# than the butt tail because in the real sequence it survives until the panel is closed.
HEAD_OVERFIT_COLOR = (1.00, 0.42, 0.22, 0.50)
HEAD_OVERFIT_EDGE_COLOR = (1.00, 0.62, 0.30, 1.0)
# The plane the head is trimmed at, drawn as a bright ring on the offcut side.
FLUSH_CUT_LINE_COLOR = (1.00, 0.95, 0.35, 1.0)
JIG_CLAMP_COLOR = (0.62, 0.66, 0.74, 1.0)
JIG_STOP_COLOR = (0.72, 0.62, 0.30, 1.0)
...
class MeshAccumulator:
    ...
    def add_quadrilateral_prism(
        ...
        """Extrude a four-sided cross section between two seam stations."""
        self.add_quad(a0, d0, c0, b0, color)
        self.add_quad(a1, b1, c1, d1, color)
        self.add_quad(a0, b0, b1, a1, color)
        self.add_quad(b0, c0, c1, b1, color)
        self.add_quad(c0, d0, d1, c1, color)
        self.add_quad(d0, a0, a1, d1, color)
...
def _member_seam(model: DomePhysicalModel, member: MemberInfo) -> SeamInfo | None:
    for seam in model.seams:
        if seam.edge_key == member.edge_key:
            return seam
    return None


def _seam_bisector_axes(seam: SeamInfo) -> tuple[np.ndarray, np.ndarray]:
    """Return (dome-out bisector, across-seam direction) in the seam cross-section."""
    bisector = normalize(seam.normal_a + seam.normal_b)
    across = normalize(np.cross(seam.tangent, bisector))
    return bisector, across


def _intersect_member_boundary_with_plane(
    member: MemberInfo,
    local: list[tuple[float, float]],
    plane_point: np.ndarray,
    plane_normal: np.ndarray,
) -> list[np.ndarray]:
    """Intersect every longitudinal boundary generator with an arbitrary end-cut plane."""
    origin = member.nominal_start + member.point_offset_in * member.inward
    denom = float(np.dot(member.tangent, plane_normal))
    if abs(denom) <= 1e-10:
        raise ValueError(f"{member.member_id} end plane is parallel to its axis")

    ring: list[np.ndarray] = []
    for y, z in local:
        line_point = origin + y * member.inward + z * member.face_normal
        t = float(np.dot(plane_point - line_point, plane_normal) / denom)
        ring.append(line_point + t * member.tangent)
    return ring


def _clip_local_polygon_to_flat_key(
    model: DomePhysicalModel,
    member: MemberInfo,
    local: list[tuple[float, float]],
) -> tuple[list[tuple[float, float]], callable | None]:
    """Longitudinally shave the seam side so it is parallel to the dihedral bisector.

    The flat key occupies a rectangular strip centered on the mathematical seam.  Each
    neighboring wood member is clipped to one side of that strip.  This is a real change
    in the rendered wood cross-section, not a decorative overlay.
    """
    seam = _member_seam(model, member)
    if seam is None or model.config.seam_join_mode != "shaved_flat":
        return local, None

    bisector, across = _seam_bisector_axes(seam)
    del bisector  # plane direction is implicit; across is its normal in cross-section.
    half_width = model.config.flat_key_width_in * 0.5
    axis_point = member.nominal_start + member.point_offset_in * member.inward
    axis_side = float(np.dot(axis_point - seam.apex_start, across))
    if abs(axis_side) <= 1.0e-8:
        # Determine side from the owning panel center if the sector point lies exactly on center.
        face = model.topology.faces[member.face_index]
        axis_side = float(np.dot(face.center - seam.apex_start, across))
    sign = 1.0 if axis_side >= 0.0 else -1.0
    cut_plane_point = seam.apex_start + across * (sign * half_width)
    keep_normal = across * sign

    def evaluate(point_yz: tuple[float, float]) -> float:
        y, z = point_yz
        gp = axis_point + y * member.inward + z * member.face_normal
        return float(np.dot(gp - cut_plane_point, keep_normal))

    # Sutherland-Hodgman clipping of the convex sector polygon against one half-plane.
    output: list[tuple[float, float]] = []
    prev = local[-1]
    prev_val = evaluate(prev)
    prev_in = prev_val >= -1.0e-9
    for curr in local:
        curr_val = evaluate(curr)
        curr_in = curr_val >= -1.0e-9
        if curr_in != prev_in:
            denom = prev_val - curr_val
            t = 0.5 if abs(denom) <= 1.0e-12 else prev_val / denom
            iy = prev[0] + (curr[0] - prev[0]) * t
            iz = prev[1] + (curr[1] - prev[1]) * t
            output.append((iy, iz))
        if curr_in:
            output.append(curr)
        prev, prev_val, prev_in = curr, curr_val, curr_in
...
def add_raw_sector_member(
    ...
    """Build one solved wood member, optionally with a longitudinal flat-key shave."""
    raw_local = _mesh_sector_local_points(model)
    local, shaved_eval = _clip_local_polygon_to_flat_key(model, member, raw_local)
    offset = np.zeros(3, dtype=np.float64) if position_offset is None else np.asarray(position_offset, dtype=np.float64)

    if shaved_eval is None:
        # Preserve the exact pre-solved raw-sector end rings in raw-key mode.
        ...
        ring_receive = [
            p + offset for p in _intersect_member_boundary_with_plane(
                member, local, member.receiving_plane_point, member.receiving_plane_normal
            )
        ]
        ring_butt = [
            p + offset for p in _intersect_member_boundary_with_plane(
                member, local, member.butt_plane_point, member.butt_plane_normal
            )
        ]
    ...
    for i in range(1, len(local) - 1):
        acc.add_triangle(ring_receive[0], ring_receive[i + 1], ring_receive[i], WEDGE_BACK_COLOR, receive_cap_normal)
        acc.add_triangle(ring_butt[0], ring_butt[i], ring_butt[i + 1], WEDGE_BACK_COLOR)

    # Longitudinal boundary surfaces.  In shaved mode the newly created parallel seam
    # face is highlighted as fresh-cut wood so it is immediately visible in the 3D world.
    n = len(local)
    for i in range(n):
        j = (i + 1) % n
        color = WEDGE_BACK_COLOR
        if shaved_eval is not None and abs(shaved_eval(local[i])) < 1.0e-6 and abs(shaved_eval(local[j])) < 1.0e-6:
            color = SHAVED_FACE_COLOR
        elif shaved_eval is None:
            if i == 0:
                color = WEDGE_LEFT_COLOR
            elif j == 0:
                color = WEDGE_RIGHT_COLOR
        acc.add_quad(ring_receive[i], ring_butt[i], ring_butt[j], ring_receive[j], color)

    # Yellow apex ridge is only drawn when the physical point remains in the clipped section.
    has_tip = any(abs(y) < 1.0e-8 and abs(z) < 1.0e-8 for y, z in local)
    if has_tip:
        tip_index = next(i for i, (y, z) in enumerate(local) if abs(y) < 1.0e-8 and abs(z) < 1.0e-8)
        tip_radius = max(0.06, model.config.trunk_diameter_in * 0.014)
        acc.add_cylinder(ring_receive[tip_index], ring_butt[tip_index], tip_radius, 10, WEDGE_TIP_COLOR)


def add_jig_butt_hangoff(
    acc: MeshAccumulator,
    model: DomePhysicalModel,
    member: MemberInfo,
    position_offset: np.ndarray | None = None,
) -> None:
    """Render the sacrificial raw-stock tail past the solved butt plane.

    The finished member is still rendered by ``add_raw_sector_member``.  This helper adds
    only the extra uncut stock that intentionally extends beyond the compound pinwheel
    butt plane.  In the physical jig that excess is not a final joint component: the jig
    fixes axis and sector rotation, the cyan butt-plane guide represents the receiver
    surface, and the excess is scribed/flush-cut away before final assembly.
    """
    hangoff = model.config.jig_butt_hangoff_in
    if hangoff <= 1.0e-9:
        return
    ...
    local, shaved_eval = _clip_local_polygon_to_flat_key(model, member, raw_local)
    if shaved_eval is None:
        butt_ring = [p.copy() for p in member.butt_contact_points]
    else:
        butt_ring = _intersect_member_boundary_with_plane(
            member, local, member.butt_plane_point, member.butt_plane_normal
        )

    # The square raw-stock end sits past the furthest point of the finished oblique cut.
    stock_ring = _member_end_stock_ring(
        model, member, local, butt_ring, hangoff, beyond=True
    )

    offset = np.zeros(3, dtype=np.float64) if position_offset is None else np.asarray(
        position_offset, dtype=np.float64)
    butt_ring = [p + offset for p in butt_ring]
    stock_ring = [p + offset for p in stock_ring]

    n = len(local)
    for i in range(n):
        j = (i + 1) % n
        acc.add_quad(butt_ring[i], stock_ring[i], stock_ring[j], butt_ring[j], JIG_HANGOFF_COLOR)

    # Square raw-stock end cap at the end of the sacrificial tail.
    for i in range(1, n - 1):
        acc.add_triangle(stock_ring[0], stock_ring[i], stock_ring[i + 1], JIG_HANGOFF_COLOR, member.tangent)


def _member_end_stock_ring(
    model: DomePhysicalModel,
    member: MemberInfo,
    local: list[tuple[float, float]],
    end_ring: list[np.ndarray],
    allowance_in: float,
    *,
    beyond: bool,
) -> list[np.ndarray]:
    """Square raw-stock end ``allowance_in`` past (or before) a solved oblique cut.

    ``beyond`` picks the direction: True walks further along the member axis, past the
    most advanced point of the cut, which is the sacrificial tail at the butt end.
    False walks backwards past the least advanced point, which is the deliberately long
    head end.  Both are raw stock, square to the axis, exactly as it comes off the
    crosscut -- that is the point of them.
    """
    origin = member.nominal_start + member.point_offset_in * member.inward
    t_values: list[float] = []
    for (y, z), point in zip(local, end_ring):
        generator_origin = origin + y * member.inward + z * member.face_normal
        t_values.append(float(np.dot(point - generator_origin, member.tangent)))
    stock_t = (max(t_values) + allowance_in) if beyond else (min(t_values) - allowance_in)
    return [
        origin + y * member.inward + z * member.face_normal + stock_t * member.tangent
        for y, z in local
    ]


def add_head_overfit(
    acc: MeshAccumulator,
    model: DomePhysicalModel,
    member: MemberInfo,
    position_offset: np.ndarray | None = None,
) -> None:
    """Render the stock that runs PAST the head end before it is flush-cut in place.

    This is the physical sequence the fabrication actually follows.  A member arrives at
    the jig with only its butt cut; its head is still long.  Three of them are pulled up
    butt-to-side, the loop closes, and only then is each head trimmed at the plane its
    neighbour has established.  Everything drawn here is the offcut.
    """
    overfit = model.config.head_overfit_in
    if overfit <= 1.0e-9:
        return
    ...
    if shaved_eval is None:
        head_ring = [p.copy() for p in member.receiving_cut_points]
    else:
        head_ring = _intersect_member_boundary_with_plane(
            member, local, member.receiving_plane_point, member.receiving_plane_normal
        )

    stock_ring = _member_end_stock_ring(
        model, member, local, head_ring, overfit, beyond=False
    )

    head_ring = [p + offset for p in head_ring]
    stock_ring = [p + offset for p in stock_ring]

    n = len(local)
    for i in range(n):
        j = (i + 1) % n
        acc.add_quad(stock_ring[i], head_ring[i], head_ring[j], stock_ring[j],
                     HEAD_OVERFIT_COLOR)

    # Square raw crosscut at the far end of the offcut.
    for i in range(1, n - 1):
        acc.add_triangle(stock_ring[0], stock_ring[i + 1], stock_ring[i],
                         HEAD_OVERFIT_COLOR, -member.tangent)


def build_flush_cut_lines(model: DomePhysicalModel, panel_index: int | None = None) -> LineData:
    """Bright ring on every head-end flush-cut plane, plus the offcut outline."""
    lines = LineAccumulator()
    local = _mesh_sector_local_points(model)
    for member in model.members:
        if panel_index is not None and member.face_index != panel_index:
            continue
        face_offset = model.topology.faces[member.face_index].normal * model.config.panel_explode_in
        ring = [p + face_offset for p in member.receiving_cut_points]
        for i in range(len(ring)):
            lines.add(ring[i], ring[(i + 1) % len(ring)], FLUSH_CUT_LINE_COLOR)
        if model.config.head_overfit_in > 1.0e-9:
            stock = _member_end_stock_ring(
                model, member, local, list(member.receiving_cut_points),
                model.config.head_overfit_in, beyond=False,
            )
            stock = [p + face_offset for p in stock]
            for i in range(len(stock)):
                lines.add(stock[i], stock[(i + 1) % len(stock)], HEAD_OVERFIT_EDGE_COLOR)
            for a, b in zip(ring, stock):
                lines.add(a, b, HEAD_OVERFIT_EDGE_COLOR)
    return lines.finish()
...
def add_rigid_spacer(acc: MeshAccumulator, model: DomePhysicalModel, seam: SeamInfo) -> None:
    """Render the selected rigid seam solution.

    raw_trapezoid:
        Keep the original radial-sawn wedge faces.  The old triangular spline is truncated
        near its exterior nose, producing the trapezoid/wedge-key section requested for
        comparing how much of the raw 45-degree sector angle remains to be absorbed.

    shaved_flat:
        The wood members are longitudinally clipped parallel to the dihedral bisector and
        a rectangular key fills the constant-width slot between those fresh-cut faces.
    """
    cfg = model.config
    if cfg.seam_join_mode == "raw_trapezoid":
        contact = max(seam.contact_depth_in, 1.0e-9)
        nose = clamp(cfg.trapezoid_nose_depth_in, 0.0, contact * 0.95)
        fraction = nose / contact
        oa0 = seam.apex_start + (seam.point_a_start - seam.apex_start) * fraction
        ob0 = seam.apex_start + (seam.point_b_start - seam.apex_start) * fraction
        oa1 = seam.apex_end + (seam.point_a_end - seam.apex_end) * fraction
        ob1 = seam.apex_end + (seam.point_b_end - seam.apex_end) * fraction
        acc.add_quadrilateral_prism(
            oa0, ob0, seam.point_b_start, seam.point_a_start,
            oa1, ob1, seam.point_b_end, seam.point_a_end,
            SPACER_COLOR,
        )
        return
    ...
    bisector, across = _seam_bisector_axes(seam)
    length = float(np.linalg.norm(seam.end - seam.start))
    depth = min(cfg.flat_key_depth_in, cfg.trunk_diameter_in * 0.75)
    center_outer = (seam.apex_start + seam.apex_end) * 0.5
    center = center_outer - bisector * (depth * 0.5)
    acc.add_box(
        center,
        seam.tangent,
        across,
...
def build_fabrication_jig(
    model: DomePhysicalModel,
    panel_index: int,
    stage: int | None = None,
) -> dict[str, MeshData | LineData]:
    """Build an exact flat assembly/cutting jig for one solved physical triangle, and
    let the builder walk through actually using it.

    ``stage`` indexes :data:`JIG_STAGES`.  Earlier stages hide fixture parts and wood
    that do not exist yet, so the fixture can be read one component at a time instead of
    appearing all at once fully loaded.  The default is the last stage, which is the
    finished panel and is what every earlier revision of this function drew.

    Nothing about the fixture is decorative.  The base is the reference plane.  The
    rails locate each member across the panel by its sawn point-axis rather than by its
    bark.  The red/green cradle plates make a wrongly-rotated sector physically refuse to
    seat.  The magenta plate is the head-end flush-cut fence and the cyan plate is the
    receiving face the next member's butt lands on -- both are generated from the same
    cut geometry as the dome itself.
    """
    panel_index %= len(model.topology.faces)
    stage = JIG_STAGE_ASSEMBLED if stage is None else max(0, min(JIG_STAGE_COUNT - 1, int(stage)))
    slug = JIG_STAGES[stage][0]

    show_triangle = stage >= jig_stage_index("triangle")
    show_rails = stage >= jig_stage_index("rails")
    show_fences = stage >= jig_stage_index("fences")
    show_guides = stage >= jig_stage_index("guides")
    show_clamps = stage >= jig_stage_index("member1")
    members_visible = {
        "buttcut": 1, "member1": 1, "member2": 2,
    }.get(slug, 3 if stage >= jig_stage_index("member3") else 0)
    # The amber butt tail exists only while the butt cut is being made, which happens
    # away from the assembly bench and before the stick is placed.
    show_butt_tail = slug == "buttcut"
    # The head stays long from the first placement until it is cut in place.
    show_overfit = jig_stage_index("buttcut") <= stage <= jig_stage_index("flush")
    # The last member of a closed pinwheel is captured at both ends, so show it backed
    # off along its own axis until the chain is pulled up tight.
    sliding_member_index = 2 if slug == "member3" else None
    highlight_flush = slug == "flush"
    highlight_contacts = slug in {"member2", "member3", "closed"}
    fixture_visible = slug != "panel"
    ...
    # Copy the exact solved wood for the selected panel into the flat jig, then add the
    # two sacrificial allowances separately.  The BUTT tail is stock left past the solved
    # receiver plane while that compound cut is being made off the jig; the HEAD overfit
    # is the stock the member still carries when it arrives, and is cut off in place at
    # the magenta fence once the pinwheel closes.
    temp_wood = MeshAccumulator()
    temp_hangoff = MeshAccumulator()
    temp_overfit = MeshAccumulator()
    staged = members[:members_visible]
    staged_ids = {m.member_id for m in staged}
    # Backing the closing member off along its own axis is a presentation move for the
    # walk-through only; it is measured from the stock so it always clears the pocket.
    slide_back_in = model.config.head_overfit_in + model.config.trunk_diameter_in
    slide_offsets: dict[str, np.ndarray] = {}
    for index, member in enumerate(staged):
        offset = None
        if sliding_member_index is not None and index == sliding_member_index:
            offset = -member.tangent * slide_back_in
            slide_offsets[member.member_id] = offset
        add_raw_sector_member(temp_wood, model, member, offset)
        if show_butt_tail:
            add_jig_butt_hangoff(temp_hangoff, model, member, offset)
        if show_overfit:
            add_head_overfit(temp_overfit, model, member, offset)
    jig_wood = _flatten_mesh(temp_wood.finish(), anchor, fx, fy, fz, wood_shift)
    jig_hangoff = _flatten_mesh(temp_hangoff.finish(), anchor, fx, fy, fz, wood_shift)
    jig_head_overfit = _flatten_mesh(temp_overfit.finish(), anchor, fx, fy, fz, wood_shift)
    ...
    for member in members:
        ...
        # axis itself is the exact repeatable placement reference; unlike the curved bark
        # side, it does not depend on bark irregularity.
        if show_rails and fixture_visible:
        ...
        # RED/GREEN orientation fences.  These are short face-parallel cradle plates that
        # physically encode which radial-sawn side of the wedge belongs on which side of
        # the jig.  This prevents a 90/180-degree sector rotation from looking "close enough"
        # while producing the wrong pinwheel receiving face or seam channel.
        red_yz = profile[1]       # tip -> first arc endpoint = RED physical radial face
        green_yz = profile[-1]    # last arc endpoint -> tip = GREEN physical radial face
        centroid_y = sum(y for y, _ in profile) / len(profile)
        centroid_z = sum(z for _, z in profile) / len(profile)
        t0 = member.receiving_axis_parameter_in
        t1 = member.butt_axis_parameter_in
        face_plate_length = min(7.0, max(3.0, float(np.linalg.norm(b - a)) * 0.15))
        face_plate_thickness = 0.16

        if show_fences and fixture_visible:
            for yz, fence_color in (
                (red_yz, JIG_RED_FACE_FENCE_COLOR),
                (green_yz, JIG_GREEN_FACE_FENCE_COLOR),
            ):
                yv, zv = yz
                radial_world = yv * member.inward + zv * member.face_normal
                radial_flat = normalize(_flat_vector(radial_world, fx, fy, fz))
                face_normal_flat = normalize(np.cross(tangent_local, radial_flat))
                # Choose the normal pointing away from the body of the sector.
                axis_mid_world = origin + ((t0 + t1) * 0.5) * member.tangent
                face_mid_world = axis_mid_world + 0.5 * radial_world
                centroid_world = axis_mid_world + centroid_y * member.inward + centroid_z * member.face_normal
                face_mid_flat = _flat_point(face_mid_world, anchor, fx, fy, fz) + wood_shift
                centroid_flat = _flat_point(centroid_world, anchor, fx, fy, fz) + wood_shift
                if float(np.dot(face_normal_flat, face_mid_flat - centroid_flat)) < 0.0:
                    face_normal_flat = -face_normal_flat

                for frac in (0.30, 0.70):
                    t_here = t0 + (t1 - t0) * frac
                    axis_here = origin + t_here * member.tangent
                    face_here = axis_here + 0.5 * radial_world
                    center = _flat_point(face_here, anchor, fx, fy, fz) + wood_shift
                    center = center + face_normal_flat * (face_plate_thickness * 0.5 + 0.05)
                    fixture.add_box(
                        center,
                        tangent_local,
                        face_normal_flat,
                        ...
                        max(1.0, float(np.linalg.norm(radial_world)) * 0.72),
                        fence_color,
                    )

        # Hold-downs.  A toggle clamp over the middle of each member is what actually
        # keeps the stick against its rail and its fences while the neighbour is pulled
        # up and while the head is being cut.  Drawn only once wood is on the bench.
        if show_clamps and fixture_visible and on_bench:
            clamp_center = (a + b) * 0.5 + inward_local * (model.config.trunk_diameter_in * 0.62)
            clamp_center[2] = wood_shift[2] + 1.6
            fixture.add_box(
                clamp_center,
                tangent_local,
                inward_local,
                np.array([0.0, 0.0, 1.0]),
                2.4,
                model.config.trunk_diameter_in * 0.55,
                3.2,
                JIG_CLAMP_COLOR,
            )
            arm_center = clamp_center - inward_local * (model.config.trunk_diameter_in * 0.34)
            arm_center[2] = wood_shift[2] + model.config.trunk_diameter_in * 0.92
            fixture.add_box(
                arm_center,
                tangent_local,
                inward_local,
                np.array([0.0, 0.0, 1.0]),
                1.6,
                model.config.trunk_diameter_in * 0.80,
                0.7,
                JIG_CLAMP_COLOR,
            )

        # Red receiving-end guide = exact clean triangle-envelope cut plane.  This plate
        # IS the flush-cut fence: a saw run flat against it trims the long head to the
        # plane the neighbouring member has already established.
        if show_guides and fixture_visible:
            ...
            # CYAN butt-cut guide = the actual receiving neighbor's PANEL_INTERIOR_FACE.
            # The orange translucent raw-stock tail deliberately crosses this plane.  In
            # real jig use this plane/fence is the flush-cut / scribe datum.
        ...
        if on_bench:
            slide = slide_offsets.get(member.member_id)

            def place(point: np.ndarray) -> np.ndarray:
                moved = point if slide is None else point + slide
                return _flat_point(moved, anchor, fx, fy, fz) + wood_shift

            rpts = [place(p) for p in member.receiving_cut_points]
            head_colour = FLUSH_CUT_LINE_COLOR if highlight_flush else JIG_VERTEX_CUT_LINE_COLOR
            for j in range(len(rpts)):
                lines.add(rpts[j], rpts[(j + 1) % len(rpts)], head_colour)

            # The butt contact polygon is only real once the member it bears on is also
            # on the bench, so it is drawn from the stage the pair first exists.
            if highlight_contacts or show_butt_tail:
                bpts = [place(p) for p in member.butt_contact_points]
                for j in range(len(bpts)):
                    lines.add(bpts[j], bpts[(j + 1) % len(bpts)], PINWHEEL_CONTACT_COLOR)

            # Outline the offcut so the amount being thrown away is legible, and so the
            # flush cut reads as a cut rather than as a shorter stick.
            if show_overfit and model.config.head_overfit_in > 1.0e-9:
                stock = _member_end_stock_ring(
                    model, member, profile, list(member.receiving_cut_points),
                    model.config.head_overfit_in, beyond=False,
                )
                spts = [place(p) for p in stock]
                for j in range(len(spts)):
                    lines.add(spts[j], spts[(j + 1) % len(spts)], HEAD_OVERFIT_EDGE_COLOR)
                for pa, pb in zip(rpts, spts):
                    lines.add(pa, pb, HEAD_OVERFIT_EDGE_COLOR)

    return {
        "jig_wood": jig_wood,
        "jig_hangoff": jig_hangoff,
        "jig_head_overfit": jig_head_overfit,
...
def build_world_meshes(model: DomePhysicalModel) -> dict[str, MeshData | LineData]:
    ...
    for member in model.members:
        ...
        add_head_overfit(head_overfit, model, member, panel_offset)

    requested_hose_radius = model.config.hose_outer_diameter_in * 0.5
    for seam in model.seams:
        add_rigid_spacer(rigid, model, seam)
    ...
    return {
        "wood": wood.finish(),
        "head_overfit": head_overfit.finish(),
        "flush_cut_lines": build_flush_cut_lines(model),
        "rigid": rigid.finish(),
        "hose": hose.finish(),
        "nodes": nodes.finish(),
        "skins": skins.finish(),
...
class ControlsHud:
    ...
    def build(
        ...
        *,
        ...
        zoom_max_steps: int,
        pan_mode: bool,
        jig_butt_hangoff_in: float,
        jig_stage: int,
        head_overfit_in: float,
    ) -> pygame.Surface:
        movement = [
        ...
        fabrication = [
            ...
            ("9 / 0", "JIG STEP back / forward"),
            ("L", "fly to jig / next corner"),
            ("Y", "next jig panel"),
            ("Shift+Y", "previous jig panel"),
            ("4", "head overfit / flush cut"),
            ("7 / 8", "head overfit - / +"),
            ("5 / 6", "butt hang-off - / +"),
            (", / .", "trunk diameter"),
            ("- / =", "nominal A length"),
        ...
        receiver_labels = {
            "point_dome_in": "BUTT->GREEN",
            "point_panel_in": "BUTT->GREEN",
            "point_dome_out": "BUTT->RED",
            "point_panel_out": "BUTT->BROWN-TAN",
        }
        receiver_label = receiver_labels.get(wedge_orientation, "BUTT->?")
        ...
        status_lines = [
            "   |   ".join(
                [
                    f"{walk_mode.upper()}",
                    f"{spacer_mode.upper()}",
                    f"JOIN {'RAW+TRAP' if seam_join_mode == 'raw_trapezoid' else 'SHAVED+FLAT'}",
            ...
            "   |   ".join(
                [
                    ...
                    f"STEP {jig_stage + 1}/{JIG_STAGE_COUNT} {JIG_STAGES[max(0, min(JIG_STAGE_COUNT - 1, jig_stage))][0].upper()}",
                    ("HEAD FLUSH-CUT" if head_overfit_in <= 0.0
                     else f"HEAD OVERFIT {head_overfit_in:.0f}in"),
                    f"BUTTTAIL {jig_butt_hangoff_in:.0f}in",
...
class SeamMinimapHud:
    """Bottom-right panel: what the two sticks either side of ONE seam actually do.

    Each of the four cells is a true cross-section through a single seam, cut
    perpendicular to the shared geodesic edge.  The seam is the vertical line down
    the middle of the cell; the left stick belongs to the panel on the left, the
    right stick to the panel on the right.

    The two sticks are NOT drawn in one shared screen frame.  Each is drawn in its
    own physical frame -- panel-inward and dome-outward -- exactly as
    ``build_physical_model`` places it, and panel-inward points in opposite screen
    directions on the two sides of a seam.  Rendering both sticks with the same
    screen rotation (the earlier behaviour) made the sideways states read as two
    sticks pointing the same way, which no seam ever looks like.  Drawn correctly
    the four states are the four things a pair of wedge points can do:

        POINT_DOME_IN    both points inward, toward the centre of the dome
        POINT_DOME_OUT   both points outward, away from the centre, at the sky
        POINT_PANEL_IN   points apart, each into the middle of its own triangle
        POINT_PANEL_OUT  points toward each other, across the seam

    The sector outline itself comes from :func:`rotate_sector_yz` at the live
    ``radial_splits``, so the picture is the same arithmetic as the wood.
    """

    READINGS = {
        "point_dome_in": ("POINTS IN", "both tips toward dome centre"),
        "point_panel_in": ("POINTS APART", "each tip into its own panel"),
        "point_dome_out": ("POINTS OUT", "both tips away from centre"),
        "point_panel_out": ("POINTS TOGETHER", "tips meet across the seam"),
    }
    ...
    def __init__(self, theme: HudTheme | None = None) -> None:
        ...
        self.bold = pygame.font.SysFont("consolas", 11, bold=True)
        self.title = pygame.font.SysFont("consolas", 12, bold=True)

    @staticmethod
    def _sector_unit_points(orientation: str, radial_splits: int,
                            segments: int = 16) -> list[tuple[float, float]]:
        """Unit-radius raw sector in LOCAL (panel-in, dome-out) coordinates."""
        half = math.radians((360.0 / max(3, radial_splits)) * 0.5)
        points: list[tuple[float, float]] = [(0.0, 0.0)]
        for i in range(segments + 1):
            angle = -half + 2.0 * half * i / segments
            points.append(
                rotate_sector_yz(math.sin(angle), math.cos(angle), orientation)
            )
        return points

    @staticmethod
    def _to_screen(points, mirror: float, scale: float,
                   cx: float, cy: float) -> list[tuple[int, int]]:
        """Map LOCAL (panel-in, dome-out) to screen pixels for one side of a seam.

        ``mirror`` is +1 for the stick whose panel lies to the RIGHT of the seam and
        -1 for the stick whose panel lies to the LEFT, because panel-inward is the
        direction that reverses between the two panels sharing an edge.  Screen-up is
        always dome-outward.
        """
        return [
            (int(round(cx + mirror * y * scale)), int(round(cy - z * scale)))
            for y, z in points
        ]
    ...
    def _draw_pair(
        ...
        bg = (10, 16, 24, 150 if active else 92)
        border = (250, 232, 120, 235) if active else (150, 160, 175, 110)
        ...
        heading, reading = self.READINGS.get(orientation, (name, ""))
        head_colour = (252, 244, 180) if active else (206, 214, 226)
        surface.blit(self.bold.render(name, True, head_colour), (rect.x + 7, rect.y + 4))
        tag = self.small.render("BUTT->" + receiver_label, True, (150, 190, 220))
        surface.blit(tag, (rect.right - tag.get_width() - 7, rect.y + 5))

        art = pygame.Rect(rect.x + 4, rect.y + 19, rect.width - 8, rect.height - 50)
        seam_x = art.centerx
        cy = art.centery + 2
        scale = min(art.width * 0.155, art.height * 0.30)

        # Dome exterior is up, dome interior is down: label the axis once per cell so
        # "in" and "out" are never guesses.
        surface.blit(self.small.render("SKY", True, (120, 150, 180)),
                     (art.centerx - 11, art.top - 1))
        surface.blit(self.small.render("CORE", True, (120, 150, 180)),
                     (art.centerx - 14, art.bottom - 9))

        # The seam itself, and a hint of the two panel surfaces meeting along it.
        pygame.draw.line(surface, (110, 190, 235, 150),
                         (seam_x, art.top + 2), (seam_x, art.bottom - 2), 1)
        pygame.draw.line(surface, (70, 105, 150, 130),
                         (art.left + 6, cy + 13), (seam_x - 2, cy + 6), 1)
        pygame.draw.line(surface, (70, 105, 150, 130),
                         (art.right - 6, cy + 13), (seam_x + 2, cy + 6), 1)

        unit = self._sector_unit_points(orientation, radial_splits)
        tip_y, tip_z = rotate_sector_yz(0.0, -1.0, orientation)

        offset = scale * 1.35
        for mirror, cx in ((-1.0, seam_x - offset), (1.0, seam_x + offset)):
            poly = self._to_screen(unit, mirror, scale, cx, cy)
            pygame.draw.polygon(surface, (150, 96, 46, 240), poly)
            pygame.draw.lines(surface, (196, 168, 120, 210), True, poly, 1)
            # RED is the seam-side radial face and GREEN the panel-side one, matching
            # the 3-D wood.  Drawn as thick edges they stay legible at this size.
            tip_px, first_px, last_px = poly[0], poly[1], poly[-1]
            pygame.draw.line(surface, (232, 46, 36), tip_px, first_px, 3)
            pygame.draw.line(surface, (46, 208, 62), last_px, tip_px, 3)
            pygame.draw.circle(surface, (252, 244, 90), tip_px, 3)

            # Arrow out of the point, so "toward" and "apart" are unmistakable.
            ax = mirror * tip_y
            az = -tip_z
            length = scale * 0.95
            end = (int(round(tip_px[0] + ax * length)),
                   int(round(tip_px[1] + az * length)))
            pygame.draw.line(surface, (252, 244, 90, 235), tip_px, end, 2)
            side = math.hypot(ax, az) or 1.0
            nx, nz = -az / side, ax / side
            head = scale * 0.26
            pygame.draw.polygon(surface, (252, 244, 90, 235), [
                end,
                (int(round(end[0] - ax * head + nx * head * 0.7)),
                 int(round(end[1] - az * head + nz * head * 0.7))),
                (int(round(end[0] - ax * head - nx * head * 0.7)),
                 int(round(end[1] - az * head - nz * head * 0.7))),
            ])

        surface.blit(self.bold.render(heading, True, head_colour),
                     (rect.x + 7, rect.bottom - 27))
        surface.blit(self.small.render(reading, True, (176, 186, 200)),
                     (rect.x + 7, rect.bottom - 14))

    def build(self, *, wedge_orientation: str, radial_splits: int = 8,
              receiver_labels: dict[str, str] | None = None) -> pygame.Surface:
        width, height = 470, 322
        surface = pygame.Surface((width, height), pygame.SRCALPHA)
        pygame.draw.rect(surface, (8, 12, 18, 96), surface.get_rect(), border_radius=10)
        pygame.draw.rect(surface, (220, 232, 242, 84), surface.get_rect(), width=1, border_radius=10)

        surface.blit(
            self.title.render("ONE SEAM, TWO STICKS  -  U CYCLES", True, (238, 244, 250)),
            (12, 7),
        )
        surface.blit(
            self.small.render(
                "cross-section across a seam - each stick drawn in its own panel's frame",
                True, (154, 166, 182)),
            (12, 23),
        )

        cells = [
            pygame.Rect(10, 40, 224, 133),
            pygame.Rect(238, 40, 222, 133),
            pygame.Rect(10, 179, 224, 133),
            pygame.Rect(238, 179, 222, 133),
        ]
        labels = receiver_labels or {}
        for rect, orientation in zip(cells, WEDGE_ORIENTATION_ORDER):
            self._draw_pair(
                surface,
                rect,
                orientation,
                radial_splits,
                labels.get(orientation, "?"),
                orientation == wedge_orientation,
            )
        return surface


class JoinSolutionHud:
    """Compact comparison of the two seam construction strategies."""

    def __init__(self, theme: HudTheme | None = None) -> None:
        self.theme = theme or HudTheme()
        pygame.font.init()
        self.font = pygame.font.SysFont("consolas", 12)
        self.bold = pygame.font.SysFont("consolas", 12, bold=True)

    def build(self, *, mode: str, edge_type: str, fold_deg: float, gap_deg: float,
              sector_angle_deg: float = 45.0) -> pygame.Surface:
        w, h = 470, 132
        surf = pygame.Surface((w, h), pygame.SRCALPHA)
        pygame.draw.rect(surf, (8, 12, 18, 88), surf.get_rect(), border_radius=9)
        pygame.draw.rect(surf, (220, 232, 242, 85), surf.get_rect(), width=1, border_radius=9)
        left = pygame.Rect(8, 8, 221, 94)
        right = pygame.Rect(241, 8, 221, 94)
        for rect, active in ((left, mode == "raw_trapezoid"), (right, mode == "shaved_flat")):
            pygame.draw.rect(surf, (18, 24, 32, 110), rect, border_radius=7)
            pygame.draw.rect(surf, (250, 250, 250, 210 if active else 80), rect, width=2 if active else 1, border_radius=7)

        # Left: two raw radial faces converge; trapezoid key fills most of the V.
        cx, cy = left.centerx, left.centery + 7
        pygame.draw.line(surf, (150, 94, 45, 255), (cx - 70, cy - 36), (cx - 12, cy + 34), 8)
        pygame.draw.line(surf, (150, 94, 45, 255), (cx + 70, cy - 36), (cx + 12, cy + 34), 8)
        trap = [(cx - 10, cy - 18), (cx + 10, cy - 18), (cx + 28, cy + 31), (cx - 28, cy + 31)]
        pygame.draw.polygon(surf, (218, 170, 72, 235), trap)
        pygame.draw.polygon(surf, (250, 240, 180, 210), trap, width=1)
        surf.blit(self.bold.render("RAW + TRAPEZOID KEY", True, (238, 244, 250)), (left.x + 12, left.y + 7))

        # Right: seam faces are shaved parallel; rectangular key fits between them.
        cx, cy = right.centerx, right.centery + 7
        pygame.draw.line(surf, (150, 94, 45, 255), (cx - 70, cy - 36), (cx - 23, cy + 34), 8)
        pygame.draw.line(surf, (150, 94, 45, 255), (cx + 70, cy - 36), (cx + 23, cy + 34), 8)
        pygame.draw.line(surf, (224, 202, 155, 255), (cx - 16, cy - 30), (cx - 16, cy + 31), 5)
        pygame.draw.line(surf, (224, 202, 155, 255), (cx + 16, cy - 30), (cx + 16, cy + 31), 5)
        pygame.draw.rect(surf, (218, 170, 72, 235), pygame.Rect(cx - 13, cy - 26, 26, 55))
        pygame.draw.rect(surf, (250, 240, 180, 210), pygame.Rect(cx - 13, cy - 26, 26, 55), width=1)
        surf.blit(self.bold.render("SHAVED + FLAT KEY", True, (238, 244, 250)), (right.x + 17, right.y + 7))

        # The raw sector angle follows radial_splits; stating 45 literally was only
        # true for an eighth-log and quietly contradicted the fold figures beside it.
        remaining = max(0.0, sector_angle_deg - fold_deg)
        text = (
            f"{edge_type}-SEAM  fold {fold_deg:5.2f}°   "
            f"included {180.0 - fold_deg:6.2f}°   "
            f"raw {sector_angle_deg:.0f}° - fold = {remaining:5.2f}°"
        )
        surf.blit(self.font.render(text, True, (210, 220, 230)), (10, 108))
        return surf


class JigStageHud:
    """Top-right panel that turns the fixture from a picture into a procedure.

    The jig used to appear all at once, fully built and fully loaded, and there was no
    way to tell which coloured plate did what.  This panel names the step you are on,
    says what you are looking at, says what that part of the fixture is physically
    enforcing, and carries a legend for every colour on the bench.  ``9`` and ``0`` step
    through it; the step list is :data:`JIG_STAGES`.
    """

    LEGEND = (
        ((0.52, 0.55, 0.58), "RAIL", "sets the stick's point-axis across the panel"),
        ((0.95, 0.10, 0.10), "RED FENCE", "the sawn face that must land on the seam side"),
        ((0.10, 0.92, 0.18), "GREEN FENCE", "the sawn face the next butt lands on"),
        ((0.95, 0.12, 0.95), "MAGENTA PLATE", "head-end FLUSH-CUT fence: saw runs on it"),
        ((0.10, 0.82, 1.00), "CYAN PLATE", "receiving face the next member's butt meets"),
        ((0.42, 0.46, 0.52), "CLAMP", "holds the stick down on rail and fences"),
        ((1.00, 0.42, 0.22), "ORANGE HEAD", "overfit stock, cut off in place -> offcut"),
        ((1.00, 0.55, 0.08), "AMBER TAIL", "butt allowance, cut before the stick arrives"),
    )

    def __init__(self, theme: HudTheme | None = None) -> None:
        self.theme = theme or HudTheme()
        pygame.font.init()
        self.title = pygame.font.SysFont("consolas", 13, bold=True)
        self.bold = pygame.font.SysFont("consolas", 11, bold=True)
        self.font = pygame.font.SysFont("consolas", 11)
        self.small = pygame.font.SysFont("consolas", 10)

    def _wrap(self, font: pygame.font.Font, text: str, width_px: int) -> list[str]:
        words = text.split()
        rows: list[str] = []
        line = ""
        for word in words:
            probe = word if not line else f"{line} {word}"
            if font.size(probe)[0] <= width_px:
                line = probe
            else:
                if line:
                    rows.append(line)
    ...
    def build(self, *, stage: int, panel_index: int, face_type: str,
              head_overfit_in: float, butt_hangoff_in: float,
              wedge_orientation: str, visible: bool) -> pygame.Surface:
        stage = max(0, min(JIG_STAGE_COUNT - 1, int(stage)))
        slug, headline, looking_at, enforcing = JIG_STAGES[stage]

        width = 430
        pad = 11
        text_w = width - pad * 2
        look_rows = self._wrap(self.font, looking_at, text_w)
        force_rows = self._wrap(self.font, enforcing, text_w)

        line_h = self.font.get_linesize()
        height = (
            pad
            + self.title.get_linesize()
            + self.small.get_linesize() + 4
            + 8
            + line_h * len(look_rows)
            + 6
            + self.bold.get_linesize()
            + line_h * len(force_rows)
            + 10
            + self.bold.get_linesize()
            + line_h * len(self.LEGEND)
            + pad
        )
        surface = pygame.Surface((width, height), pygame.SRCALPHA)
        pygame.draw.rect(surface, (8, 12, 18, 116), surface.get_rect(), border_radius=9)
        pygame.draw.rect(surface, (220, 232, 242, 90), surface.get_rect(), width=1, border_radius=9)

        y = pad
        surface.blit(self.title.render(headline, True, (250, 238, 170)), (pad, y))
        y += self.title.get_linesize()

        status = (
            f"step {stage + 1}/{JIG_STAGE_COUNT}   9/0 to step   T {'hides' if visible else 'shows'} jig"
            f"   panel P{panel_index + 1:03d}/{face_type}"
        )
        surface.blit(self.small.render(status, True, (150, 164, 180)), (pad, y))
        y += self.small.get_linesize() + 4

        # progress bar across the steps
        bar = pygame.Rect(pad, y, text_w, 4)
        pygame.draw.rect(surface, (48, 56, 66), bar, border_radius=2)
        done = pygame.Rect(pad, y, int(text_w * (stage + 1) / JIG_STAGE_COUNT), 4)
        pygame.draw.rect(surface, (250, 200, 90), done, border_radius=2)
        y += 12

        for row in look_rows:
            surface.blit(self.font.render(row, True, (226, 234, 244)), (pad, y))
            y += line_h
        y += 6

        surface.blit(self.bold.render("WHAT THIS ENFORCES", True, (140, 210, 255)), (pad, y))
        y += self.bold.get_linesize()
        for row in force_rows:
            surface.blit(self.font.render(row, True, (186, 198, 212)), (pad, y))
            y += line_h
        y += 10

        allowances = (
            f"head overfit {head_overfit_in:.0f}\"  ·  butt tail {butt_hangoff_in:.0f}\"  ·  "
            f"{wedge_orientation.replace('point_', '').replace('_', '-').upper()}"
        )
        surface.blit(self.bold.render(allowances, True, (200, 212, 226)), (pad, y))
        y += self.bold.get_linesize()

        for rgb, name, meaning in self.LEGEND:
            swatch = pygame.Rect(pad, y + 3, 9, 9)
            pygame.draw.rect(surface, tuple(int(c * 255) for c in rgb), swatch, border_radius=2)
            surface.blit(self.small.render(name, True, (222, 230, 240)), (pad + 15, y))
            surface.blit(self.small.render(meaning, True, (158, 170, 186)), (pad + 120, y))
            y += line_h
...
class DomeWorldApp:
    ...
    def __init__(self) -> None:
        ...
        self.join_hud = JoinSolutionHud()
        self.stage_hud = JigStageHud()
        ...
        self.jig_stage = self.config.jig_stage
        self.jig_view_index = -1
        self.solo_panel = False
        # Head-end stock is on by default, because the whole point of the sequence is
        # that the head arrives long and is trimmed in place.  Toggling it off shows the
        # finished, flush-cut member so the two states can be compared directly.
        self.show_head_overfit = True
        self.running = True
    ...
    def _refresh_hud(self, force: bool = False) -> None:
        """Rebuild controls plus orientation and seam-solution diagrams when state changes."""
        ...
        selected = self.model.seams[self.selected_seam % len(self.model.seams)]
        state = (
            "fly" if self.camera.fly_mode else "walk",
            self.config.spacer_mode,
            self.config.seam_join_mode,
            self.mouse_captured,
            ...
            self.zoom_max_steps,
            self.in_pan_mode(),
            self.config.jig_butt_hangoff_in,
            self.selected_seam,
            self.jig_stage,
            self.show_head_overfit,
            self.config.head_overfit_in,
            self.config.radial_splits,
        )
        ...
        controls_surface = self.hud.build(
            ...
            seam_join_mode=str(state[2]),
            mouse_captured=bool(state[3]),
            pinwheel_handedness=str(state[4]),
            wedge_orientation=str(state[5]),
            jig_visible=bool(state[6]),
            jig_panel=int(state[7]),
            jig_face_type=str(state[8]),
            vertex_trim_mode=str(state[9]),
            panel_explode_in=float(state[10]),
            solo_panel=bool(state[11]),
            seam_open_a_in=float(state[12]),
            seam_open_b_in=float(state[13]),
            zoom_steps=int(state[14]),
            zoom_max_steps=int(state[15]),
            pan_mode=bool(state[16]),
            jig_butt_hangoff_in=float(state[17]),
            jig_stage=int(state[19]),
            head_overfit_in=float(state[21]) if bool(state[20]) else 0.0,
        )
        minimap_surface = self.minimap_hud.build(
            wedge_orientation=str(state[5]),
            radial_splits=self.config.radial_splits,
            receiver_labels={
                key: pinwheel_receiver_surface_label(replace(self.config, wedge_orientation=key))
                for key in WEDGE_ORIENTATION_ORDER
            },
        )
        join_surface = self.join_hud.build(
            mode=self.config.seam_join_mode,
            edge_type=selected.edge_type,
            fold_deg=selected.fold_angle_deg,
            gap_deg=selected.raw_gap_angle_deg,
            sector_angle_deg=360.0 / self.config.radial_splits,
        )
        stage_surface = self.stage_hud.build(
            stage=int(state[19]),
            panel_index=int(state[7]),
            face_type=str(state[8]),
            head_overfit_in=float(state[21]),
            butt_hangoff_in=float(state[17]),
            wedge_orientation=str(state[5]),
            visible=bool(state[6]),
        )
    ...
    def _upload_jig(self) -> None:
        ...
        self.jig_stage = max(0, min(JIG_STAGE_COUNT - 1, self.jig_stage))
        self.config.jig_stage = self.jig_stage
        self.jig_cpu = build_fabrication_jig(self.model, self.jig_panel, self.jig_stage)
        for name in ("jig_wood", "jig_hangoff", "jig_head_overfit", "jig_fixture"):
    ...
    def rebuild_world(self, reset_camera: bool = False) -> None:
        ...
        for name in ("wood", "head_overfit", "rigid", "hose", "nodes", "skins"):
            data = self.world_cpu[name]
            assert isinstance(data, MeshData)
            self.renderer.upload_mesh(name, data)
        for name in ("ideal", "ground", "joint_debug", "flush_cut_lines"):
            data = self.world_cpu[name]
    ...
    def jig_viewpoints(self) -> list[tuple[str, np.ndarray, np.ndarray]]:
        """Camera stations around the fixture, so the jig can be inspected without
        having to fly to it by hand. One overview plus one per corner joint, because
        the corner is where the whole method either makes sense or does not."""
        anchor, fx, fy, fz = _panel_flat_frame(self.model, self.jig_panel)
        shift = fabrication_jig_world_shift(self.model, self.jig_panel)
        face = self.model.topology.faces[self.jig_panel]
        corners = [
            _flat_point(self.model.topology.vertices[i], anchor, fx, fy, fz) + shift
            for i in face.vertices
        ]
        centre = sum(corners) / 3.0
        span = max(float(np.linalg.norm(a - b)) for a in corners for b in corners)

        stations: list[tuple[str, np.ndarray, np.ndarray]] = [
            (
                "WHOLE JIG",
                centre + np.array([-span * 0.55, -span * 0.70, span * 0.72]),
                centre,
            )
        ]
        for i, corner in enumerate(corners):
            outward = normalize(corner - centre)
            eye = corner + outward * (self.config.trunk_diameter_in * 3.4)
            eye[2] = shift[2] + self.config.trunk_diameter_in * 2.6
            stations.append((f"CORNER {i + 1}", eye, corner))
        return stations

    def go_to_jig_view(self, step: int = 1) -> None:
        stations = self.jig_viewpoints()
        self.jig_view_index = (self.jig_view_index + step) % len(stations)
        name, eye, target = stations[self.jig_view_index]
        self.camera.position = np.asarray(eye, dtype=np.float64)
        direction = normalize(np.asarray(target, dtype=np.float64) - self.camera.position)
        self.camera.yaw_deg = math.degrees(math.atan2(direction[1], direction[0]))
        self.camera.pitch_deg = math.degrees(math.asin(clamp(float(direction[2]), -1.0, 1.0)))
        self.camera.fly_mode = True
        self.show_jig = True
        self.zoom_steps = 0
        self._refresh_hud(force=True)
        print(f"JIG VIEW: {name}")
    ...
    def update_caption(self, fps: float) -> None:
        ...
        title = (
            ...
            f"spacer={spacer} | join={self.config.seam_join_mode.upper()} | A={self.model.topology.long_edge_in:.2f}\" "
...
M                        Toggle RAW+TRAPEZOID / SHAVED+FLAT seam construction
...
[ / ]                    Previous / next seam
1 / 2                    Jump directly to an A-family / B-family seam
T                        Toggle fabrication jig
9 / 0                    Step the jig walk-through backward / forward
L / Shift+L              Jump the camera to the jig, then to each corner joint
Y                        Next exact physical panel on jig
Shift+Y                  Previous exact physical panel on jig
4                        Head end: overfit stock ON (before flush cut) / OFF (after)
7 / 8                    Decrease / increase head-end overfit by 1 in
5 / 6                    Decrease / increase sacrificial butt hang-off by 1 in
...
THE TWO ENDS ARE NOT THE SAME OPERATION
  BUTT end  cut BEFORE assembly, off the jig, to the compound angle its
            receiving neighbour presents. Same cut for every stick of a family.
  HEAD end  cut AFTER assembly, in place, at the plane the neighbour has already
            established. It arrives long on purpose, so stock-length and butt-angle
            error leaves as offcut instead of accumulating around the triangle.
  Press 4 to switch between the long head and the flush-cut head; 9 / 0 walk the
  eleven steps of building and using the jig.
"""
        )
    ...
    def handle_keydown(self, key: int) -> None:
        if key == pygame.K_ESCAPE:
            ...
            self.config.seam_join_mode = (
                "shaved_flat" if self.config.seam_join_mode == "raw_trapezoid" else "raw_trapezoid"
            )
            self.rebuild_world()
        elif key == pygame.K_i:
            ...
            self.config.jig_butt_hangoff_in = max(0.0, self.config.jig_butt_hangoff_in - 1.0)
            self._upload_jig()
        elif key == pygame.K_6:
            self.config.jig_butt_hangoff_in = min(24.0, self.config.jig_butt_hangoff_in + 1.0)
            self._upload_jig()
        elif key == pygame.K_l:
            direction = -1 if (pygame.key.get_mods() & pygame.KMOD_SHIFT) else 1
            self.go_to_jig_view(direction)
        elif key == pygame.K_4:
            # Before / after the head-end flush cut.
            self.show_head_overfit = not self.show_head_overfit
            self._refresh_hud(force=True)
        elif key == pygame.K_7:
            self.config.head_overfit_in = max(0.0, self.config.head_overfit_in - 1.0)
            self.rebuild_world()
        elif key == pygame.K_8:
            self.config.head_overfit_in = min(24.0, self.config.head_overfit_in + 1.0)
            self.rebuild_world()
        elif key in (pygame.K_9, pygame.K_0):
            step = -1 if key == pygame.K_9 else 1
            self.jig_stage = max(0, min(JIG_STAGE_COUNT - 1, self.jig_stage + step))
            self.show_jig = True
            self._upload_jig()
            print(f"JIG STEP {self.jig_stage + 1}/{JIG_STAGE_COUNT}: {JIG_STAGES[self.jig_stage][1]}")
            print(f"    {JIG_STAGES[self.jig_stage][2]}")
            print(f"    ENFORCES: {JIG_STAGES[self.jig_stage][3]}")
        elif key in (pygame.K_1, pygame.K_2):
            target_type = "A" if key == pygame.K_1 else "B"
            self.selected_seam = next(
                i for i, seam in enumerate(self.model.seams) if seam.edge_type == target_type
            )
            self.renderer.upload_lines("selected", build_selected_seam_lines(self.model, self.selected_seam))
            self._refresh_hud(force=True)
    ...
    def render(self) -> None:
        ...
        if self.show_wood:
            ...
            # The stock that runs past every head end before it is cut in place. Seeing it
            # in the assembled dome is the argument for the flush cut: uncut, the heads
            # trespass straight across the geodesic vertex into the neighbouring panel.
            if self.show_head_overfit and not self.solo_panel:
                self.renderer.render_mesh("head_overfit", view_projection)
                self.renderer.render_lines("flush_cut_lines", view_projection, 1.6)

        # Seam-owned connectors are anchored to the mathematical dihedral joining line.
        # Panel explosion moves ONLY the complete triangle frames.  The rigid trapezoid /
        # flat key (and hose when selected) remains at its true assembled seam location so
        # the user can pull the panels away and inspect exactly how the connector occupies
        # the missing angle.  Node markers still hide while exploded because they are not
        # the current analysis target.
        if self.show_spacer and not self.solo_panel:
        ...
        assembled_node_view = self.config.panel_explode_in <= 1.0e-9 and not self.solo_panel
        if self.show_nodes and assembled_node_view:
        ...
        if self.show_jig:
            ...
            self.renderer.render_mesh("jig_hangoff", view_projection)
            if self.show_head_overfit:
                self.renderer.render_mesh("jig_head_overfit", view_projection)
        ...
        if self.show_hud:
            ...
            stage_overlay = self.renderer.overlays.get("jig_stage")
            if stage_overlay is not None:
                self.renderer.render_overlay(
                    "jig_stage",
                    self.width,
                    self.height,
                    x_px=max(self.hud.theme.margin_px,
                             self.width - stage_overlay.width - self.hud.theme.margin_px),
                    y_px=self.hud.theme.margin_px,
                )
            seam_overlay = self.renderer.overlays.get("seam_minimap")
            join_overlay = self.renderer.overlays.get("join_solution")
            if seam_overlay is not None:
                seam_x = max(self.hud.theme.margin_px, self.width - seam_overlay.width - self.hud.theme.margin_px)
                seam_y = max(self.hud.theme.margin_px, self.height - seam_overlay.height - self.hud.theme.margin_px)
                self.renderer.render_overlay(
                    "seam_minimap", self.width, self.height, x_px=seam_x, y_px=seam_y
                )
                if join_overlay is not None:
                    self.renderer.render_overlay(
                        ...
                        self.width,
                        self.height,
                        x_px=max(self.hud.theme.margin_px, self.width - join_overlay.width - self.hud.theme.margin_px),
...
# ============================================================================
# What the pair of wedge points either side of ONE seam physically does, in each of the
# four states. Shared by the console validation and the on-screen diagram so the two can
# never disagree about which state is which.
SEAM_PAIR_READING = {
    "point_dome_in": "both points inward, toward the centre of the dome",
    "point_panel_in": "points apart, each into the middle of its own triangle",
    "point_dome_out": "both points outward, away from the centre, at the sky",
...
def run_geometry_validation() -> None:
    ...
    print("SEAM CONSTRUCTION MODES — POINT_DOME_IN")
    print("-" * 72)
    for join_mode in ("raw_trapezoid", "shaved_flat"):
        mode_cfg = DomeConfig(wedge_orientation="point_dome_in", seam_join_mode=join_mode)
        mode_model = build_physical_model(mode_cfg)
        mode_world = build_world_meshes(mode_model)
        assert len(mode_world["wood"].indices) > 0
        assert len(mode_world["rigid"].indices) > 0
        a = next(seam for seam in mode_model.seams if seam.edge_type == "A")
        b = next(seam for seam in mode_model.seams if seam.edge_type == "B")
        print(
            f"  {join_mode.upper():14s}  A fold={a.fold_angle_deg:.6f}°  "
            f"45-fold={45.0-a.fold_angle_deg:.6f}°  "
            f"B fold={b.fold_angle_deg:.6f}°  45-fold={45.0-b.fold_angle_deg:.6f}°"
        )
    print()

    print("HEAD-END OVERFIT AND FLUSH CUT")
    print("-" * 72)
    print("The butt end is cut before assembly; the head end is left long and cut in")
    print("place. These are the two allowances, and what each one is worth per dome.")
    for orientation in WEDGE_ORIENTATION_ORDER:
        oc = DomeConfig(wedge_orientation=orientation)
        om = build_physical_model(oc)
        finished = sum(m.physical_stock_length_in for m in om.members)
        offcut = oc.head_overfit_in * len(om.members)
        print(
            f"  {orientation.upper():16s} finished stock {finished / 12.0:8.2f} ft   "
    ...
    print("-" * 72)
    stage_model = build_physical_model(DomeConfig())
    for index, (slug, headline, _looking, _forces) in enumerate(JIG_STAGES):
        parts = build_fabrication_jig(stage_model, 5, index)
        wood_tris = len(parts["jig_wood"].indices) // 3
        head_tris = len(parts["jig_head_overfit"].indices) // 3
        tail_tris = len(parts["jig_hangoff"].indices) // 3
        fixture_tris = len(parts["jig_fixture"].indices) // 3
        print(
            f"  {index + 1:2d}/{JIG_STAGE_COUNT} {slug:9s} "
            f"fixture {fixture_tris:4d}  wood {wood_tris:4d}  "
            f"head {head_tris:4d}  butt-tail {tail_tris:4d}   {headline}"
        )
    # The butt tail belongs to the pre-assembly cut only, and the heads must be gone
    # from the finished panel; both are easy to get backwards, so assert them.
    bench = build_fabrication_jig(stage_model, 5, jig_stage_index("closed"))
    assert len(bench["jig_hangoff"].indices) == 0, "butt tail must not be on the assembly bench"
    assert len(bench["jig_head_overfit"].indices) > 0, "heads must still be long when the loop closes"
    done = build_fabrication_jig(stage_model, 5, jig_stage_index("panel"))
    assert len(done["jig_head_overfit"].indices) == 0, "heads must be cut off the finished panel"
    assert len(done["jig_fixture"].indices) == 0, "finished panel leaves the fixture behind"
    print()

    print("ORIENTATION READINGS — WHAT A PAIR OF POINTS AT ONE SEAM DOES")
    print("-" * 72)
    for orientation in WEDGE_ORIENTATION_ORDER:
        point_y, point_z = sector_point_direction_local(
            DomeConfig(wedge_orientation=orientation)
        )
        reading = SEAM_PAIR_READING[orientation]
        print(
            f"  {orientation.upper():16s} point -> panel-in {point_y:+.3f}  "
            f"dome-out {point_z:+.3f}   {reading}"
        )
    print()

    print("PASS: all four wedge rotations regenerate 120 members, clean vertex cuts,")
    print("      cyclic panel joints, 55 seams, both rigid seam strategies, the twelve-step")
    print("      jig walk-through, and the head-end overfit / flush-cut sequence.")
...
# New joinery addendum appended to the embedded master specification so
# --extract-resources preserves the seam-strategy work added after the original flattening.
MASTER_BUILD_SPEC_MD += r"""

---

# SEAM JOINERY ADDENDUM — RAW TRAPEZOID KEY VS SHAVED FLAT KEY

The 2V dome contains two interior fold families generated directly from face normals:

```text
A-family fold ≈ 18.029102°   included face angle ≈ 161.970898°
B-family fold ≈ 22.458924°   included face angle ≈ 157.541076°
```

For the legacy POINT_DOME_IN orientation of an eight-way split log sector, the two raw
radial seam faces contribute a 45° total raw-sector relationship. The useful comparison is:

```text
A family: 45° - 18.029102° = 26.970898° remaining
B family: 45° - 22.458924° = 22.541076° remaining
```

The application now exposes two construction strategies through `seam_join_mode` and the
`M` hotkey.

## RAW_TRAPEZOID

- Do not longitudinally shave the raw wedge seam faces.
- Preserve the natural one-eighth-log radial faces.
- Insert a truncated wedge / trapezoid key between neighboring panel members.
- The key is generated from the actual two raw seam-face planes and therefore follows the
  generated A/B fold geometry rather than a hard-coded bevel.
- `trapezoid_nose_depth_in` controls how far the old triangular key is truncated from its
  virtual nose; zero reproduces a triangular full-depth key, while a positive value creates
  a genuine trapezoidal section.

## SHAVED_FLAT

- Keep the panel orientation and geodesic fold unchanged.
- Longitudinally clip each neighboring wedge's seam side so the two fresh-cut contact faces
  are parallel to the dihedral-bisector direction.
- A rectangular flat key then occupies the constant-width slot between those two parallel
  faces.
- `flat_key_width_in` controls the slot/key width across the seam.
- `flat_key_depth_in` controls radial key depth from the exterior ridge inward.
- The fresh longitudinal wood faces are rendered as light fresh-cut wood so material removal
  can be seen directly in the 3D world.

The bottom-right join-solution overlay shows both strategies simultaneously. The active one
is outlined more strongly. The selected seam's A/B family, fold angle, supplementary included
angle, and 45°-minus-fold comparison are shown beneath the diagrams.

Controls:

```text
M       toggle RAW_TRAPEZOID / SHAVED_FLAT
1       jump directly to an A-family seam
2       jump directly to a B-family seam
[ / ]   cycle individual seams
H       rigid key / hose / no spacer
```

Fabrication export now additionally produces:

```text
SEAM_JOIN_SCHEDULE.csv
```

with every seam's fold, included angle, raw-sector reference, remaining-angle comparison,
trapezoid widths, flat-key dimensions, seam length, and active construction mode.
"""

# Jig / exploded-seam addendum.  Kept separate so --extract-resources reproduces the
# newest construction rules even though the original master spec is embedded above.
MASTER_BUILD_SPEC_MD += r"""

---

# SEAM-OWNED KEY + SACRIFICIAL BUTT-HANGOFF JIG ADDENDUM

## Keys do not explode with panels

The rigid trapezoid key, shaved-flat rectangular key, and hose are **SEAM-OWNED** objects.
They are anchored to the mathematical shared-edge / dihedral-bisector solution and are not
children of either adjacent triangle panel.  `panel_explode_in` translates only complete
triangle frames along their own face normals.  Therefore pressing X must pull the two frames
away while leaving the selected connector fixed at its assembled seam position.  This is
intentional: the exploded view is supposed to reveal the connector's neutral location and
show how much angle/space the two frame members leave for it.

## Pinwheel receiving surface depends on wedge rotation

The colored wedge faces are physical identities, not screen decoration:

```text
POINT_DOME_IN   -> pinwheel butt terminates against GREEN radial face
POINT_PANEL_IN  -> pinwheel butt terminates against GREEN radial face
POINT_DOME_OUT  -> pinwheel butt terminates against RED radial face
POINT_PANEL_OUT -> panel-interior receiver is curved BROWN surface; tangent datum only
```

The jig must preserve these identities.  Red/green orientation fences are therefore part of
the fixture, not merely labels.

## Butt end is cut after alignment, with sacrificial raw-stock hang-off

The finished solid model still stores the exact compound pinwheel butt plane.  The fabrication
jig additionally visualizes a raw-stock tail extending beyond that plane by
`jig_butt_hangoff_in` (default 8 inches).  The finished wood is shown normally; the sacrificial
excess is translucent orange.

The uncut tail is **not** intended to coexist physically inside the receiving wood.  During
cutting, the cyan guide plate acts as a receiver-face proxy.  The workflow is:

1. Place the raw wedge on the gray axis/location rail.
2. Seat it into the RED and GREEN orientation fences so the sector cannot roll 90°/180°.
3. Let the raw butt end extend past the cyan receiver-face proxy plane.
4. Scribe or flush-cut the orange sacrificial tail at the cyan plane.
5. Remove the waste.
6. Dry-fit the actual receiving member in the same enforced jig position.
7. Repeat around the three-member cyclic pinwheel.
8. After all three butt cuts are complete, all three finished members can occupy the jig
   simultaneously without impossible solid overlap.

The magenta guide remains the separate clean geodesic-vertex receiving-end cut.

Controls:

```text
5 / 6   decrease / increase sacrificial butt hang-off by 1 inch
T       show / hide fabrication jig
Y       next panel jig
Shift+Y previous panel jig
```

Fabrication export includes `JIG_CUT_SEQUENCE.csv`, updated member cut schedules, SVG panel
jigs, fixture OBJ files, finished example wood OBJ files, and separate sacrificial butt-tail
OBJ files.
"""
...
# Fields the launcher is allowed to set on the saved config before the world opens.
# Anything not listed here is left exactly as the tool's own config file has it.
TICKET_CONFIG_FIELDS = (
    "long_edge_in",
    "trunk_diameter_in",
    "radial_splits",
    "arc_segments",
...
def apply_launch_ticket(ticket: dict) -> None:
    """Fold a launcher ticket into the saved config so the app opens configured.

    The tool has no command-line surface for these; the launcher writes a one-shot JSON
    ticket and this merges it. Running the file directly finds no ticket and every value
    stays as the config file left it.
    """
    if not ticket:
        return
    config = DomeConfig.load(CONFIG_PATH) if CONFIG_PATH.exists() else DomeConfig()
    changed = False
    for field_name in TICKET_CONFIG_FIELDS:
        if field_name not in ticket or ticket[field_name] in (None, ""):
            continue
        ...
        try:
            ...
            print(f"Ignoring launcher value for {field_name}: {value!r}")
            continue
    ...
    print(f"Applied launcher settings to {CONFIG_PATH.name}")


def _read_launch_ticket() -> dict:
    """Consume this tool's launcher ticket, if the launcher wrote one."""
    try:
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        import launcher_common  # noqa: PLC0415 - optional, only present in the project
    except Exception:
        return {}
    try:
        return launcher_common.consume_config("raw_wedge_dome")
    except Exception as exc:  # noqa: BLE001 - a bad ticket must not stop the tool
        print(f"Could not read launcher ticket: {exc}")
        return {}
...
def _single_file_cli() -> None:
    ...
    action = str(ticket.get("action", "")).strip().lower()
    apply_launch_ticket(ticket)

    if args.validate or action == "validate":
        run_geometry_validation()
        return
    if args.fabrication or action == "fabrication":
