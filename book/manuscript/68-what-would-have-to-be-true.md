...
| the brim catches {{brim.gallons_yr}} gallons | declared rainfall and runoff | computed |
...
{{dome.diameter_ft}} foot dome from two trees, the {{work.days}} days of
...
{{ch.corrections}}) exists because the project has been wrong before; this
...
taxonomy of everything they teach -- is {{honest.concepts}} concepts, each
with a claim, a caveat and a source. The films' own summary is
{{honest.points}} points. A book this careful about those numbers owes the
reader the same care in reverse: this chapter is the {{honest.points}}
points and the {{honest.concepts}} concepts, restated as conditions, so that
...
{{honest.finished_figure}}: pressure treated, insulated, glassed and fitted
out, at {{honest.finished_per_sqft}} dollars a square foot. That is the
number the shape argument wants you to remember, and it is a real number --
{{honest.finished_detail}}.

Now the number next to it. The national average new home sells for
{{honest.price}} dollars, of which {{honest.construction}} is the building
cost -- {{honest.per_sqft}} dollars a square foot over
{{honest.nahb_sqft}} square feet. Put the dome's {{honest.ref_floor}} square
foot reference floor through that rate and it comes to
{{honest.national_floor}} dollars. The gap between the two ways of pricing
one floor is {{honest.gap}} dollars, and the national rate is
{{honest.ratio}} times the finished figure.

Neither number is a lie, and that is exactly why both get printed. The
{{honest.finished_per_sqft}} dollar figure counts the *shape* -- the frame,
the skin, the glass, the minimal fit-out the costing module knows about. The
{{honest.per_sqft}} dollar figure counts a *house* -- foundations to code,
...
gap is this chapter's: {{honest.gap}} dollars between two real numbers, with
...
-- {{skin.heat_pct}} per cent less surface at the reference floor, less
...
**Built.** A dome is standing: the {{dome.diameter_ft}} foot frame from two
trees, {{work.days}} days of the fortnight, {{value.hours}} hours of one
person's hands on it. The frame the films show is the frame that exists.

**Measured.** The fortnight's money is real: {{value.frame_cash}} dollars
spent against {{value.frame_buy}} dollars the same frame would have cost
bought -- {{value.rate}} dollars an hour for the builder, which is what the
log actually paid.
