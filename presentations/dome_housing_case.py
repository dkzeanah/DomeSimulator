...
# ---------------------------------------------------------------------------
# Real numbers, pulled once from the live modules the interactive tools use.
# ---------------------------------------------------------------------------
...
def _pct_less(a: float, b: float) -> float:
    """Percent by which ``b`` is less than ``a``."""
    return 100.0 * (1.0 - b / a)


SHED_SAVE_PCT = _pct_less(SHED["box"]["build"], SHED["dome"]["build"])
SHED_FRAME_PCT = _pct_less(SHED["box"]["framing_lf"], SHED["dome"]["framing_lf"])
SHED_CLAD_PCT = _pct_less(SHED["box"]["cladding_sf"], SHED["dome"]["cladding_sf"])
HOME_SAVE_PCT = _pct_less(HOME["box"]["build"], HOME["dome"]["build"])
HOME_FRAME_PCT = _pct_less(HOME["box"]["framing_lf"], HOME["dome"]["framing_lf"])
HOME_VOL_PCT = 100.0 * (HOME["dome"]["vol_ft3"] / HOME["box"]["vol_ft3"] - 1.0)

R = 4.2  # hero dome radius for the teaching/build scenes — inside the real
         # product line's own "home" radius range of 3.2-4.6 m.
...
BENCH_PRICE_PCT = _pct_less(HERO_BENCH["conventional"]["price"],
                            HERO_BENCH["dome"]["price"])
BENCH_LABOR_PCT = _pct_less(HERO_BENCH["conventional"]["labor_hours"],
                            HERO_BENCH["dome"]["labor_hours"])
BENCH_DAYS_PCT = _pct_less(HERO_BENCH["conventional"]["build_days"],
