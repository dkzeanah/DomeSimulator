...
const RENDER_DIR = path.join(RUN_DIR, "renders", "v005");
const OUTPUT_PPTX = path.join(
  RUN_DIR,
  "wheelchair_first_dome_living_20260816_082305_v005.pptx",
);
...
function repaintHeaderIndented(slide, title, number, eyebrow = "WHEELCHAIR-FIRST DOME LIVING") {
  addShape(slide, { left: 0, top: 0, width: W, height: 174 }, C.white, "rect");
  addText(slide, title, { left: 150, top: 70, width: 1088, height: 74 }, {
    fontSize: 47,
    bold: true,
    color: C.ink,
    autoFit: "shrinkText",
  });
  addShape(slide, { left: 42, top: 158, width: 1196, height: 1 }, C.rule, "rect", null, `indented-top-rule-${number}`);
  addText(slide, eyebrow, { left: 42, top: 30, width: 460, height: 24 }, {
    fontSize: 14,
    bold: true,
    color: C.blueDeep,
...
async function main() {
  ...
  // 2 — image-led case for one level.
  {
    ...
    repaintHeader(slide, "One level changes the whole day", 2);
    setNotes(
      slide,
  ...
  // 3 — movement map and radial planning.
  {
    ...
    repaintHeader(slide, "Start with a turning circle, then place the rooms", 3);
    setNotes(
      slide,
  ...
  // 5 — why the dome form helps.
  {
    ...
    repaintHeader(slide, "The shell can organize space around movement", 5);
    setNotes(
      slide,
  ...
  {
    const slide = baseSlide(presentation);
    addText(slide, "06", { left: 1187, top: 672, width: 52, height: 20 }, {
      fontSize: 14,
      color: C.muted,
      alignment: "right",
    });
    ...
    repaintHeaderIndented(slide, "The entrance is the first independence test", 6);
    setNotes(
      slide,
  ...
  // 8 — bed and bath.
  {
    ...
    repaintHeader(slide, "Privacy depends on space to move", 8);
    setNotes(
      slide,
  ...
  // 9 — three benchmark metrics, Codex Grid metric-led composition.
  {
    ...
    const metrics = [
      ...
      ["32 in", "CLEAR OPENING", "Minimum clear width at a door opened to 90 degrees."],
      ["1:12", "MAXIMUM RAMP SLOPE", "Ramp benchmark; gentler is better where practical."],
    ];
    metrics.forEach(([stat, label, body], index) => {
      ...
      addText(slide, body, { left: left + 30, top: 519, width: 292, height: 58 }, { fontSize: 19, color: C.muted, autoFit: "shrinkText" });
    });
  ...
  // 10 — aging and fall stakes.
  {
    ...
    repaintHeader(slide, "Wheelchair flow also protects aging bodies", 10);
    setNotes(
      slide,
