...
const RENDER_DIR = path.join(RUN_DIR, "renders", "v001");
const OUTPUT_PPTX = path.join(
  RUN_DIR,
  "wheelchair_first_dome_living_20260816_082305_v001.pptx",
);
...
async function main() {
  ...
  // 3 — movement map and radial planning.
  {
    ...
    steps.forEach(([n, label, body], index) => {
      ...
      addText(slide, body, { left: 235, top, width: 215, height: 72 }, { fontSize: 19, color: C.muted });
    });
    addImage(
      slide,
      assets.cutaway,
  ...
  // 4 — daily barriers, sparse four-point composition.
  {
    ...
    taxes.forEach(([word, body], index) => {
      ...
      addText(slide, body, { left, top: 468, width: 245, height: 94 }, { fontSize: 21, color: C.muted });
    });
  ...
  // 6 — entry benchmarks.
  {
    ...
    addText(slide, "Benchmark from the ADA Standards; local residential requirements and individual needs may differ.", { left: 42, top: 464, width: 320, height: 92 }, { fontSize: 18, color: C.muted });
  ...
  // 9 — three benchmark metrics, Codex Grid metric-led composition.
  {
    ...
    metrics.forEach(([stat, label, body], index) => {
      ...
      addText(slide, label, { left: left + 30, top: 472, width: 294, height: 34 }, { fontSize: 18, bold: true, color: C.blueDeep });
      addText(slide, body, { left: left + 30, top: 523, width: 292, height: 62 }, { fontSize: 18, color: C.muted });
    });
    setNotes(
      slide,
  ...
  // 11 — explicit conventional vs dome comparison.
  {
    ...
    addText(slide, "TYPICAL CONVENTIONAL RETROFIT", { left: 42, top: 206, width: 510, height: 34 }, { fontSize: 19, bold: true, color: C.muted });
    addText(slide, "PURPOSE-BUILT ACCESSIBLE DOME", { left: 684, top: 206, width: 510, height: 34 }, { fontSize: 19, bold: true, color: C.blueDeep });
  ...
  // 12 — honest close and action.
  {
    ...
    addShape(slide, { left: 850, top: 350, width: 345, height: 236 }, C.bluePale, "roundRect");
    addText(slide, "NEXT STEP", { left: 884, top: 384, width: 280, height: 28 }, { fontSize: 17, bold: true, color: C.blueDeep });
    addText(slide, "Map one real user’s day—entry, turn, reach, transfer—before pricing the shell.", { left: 884, top: 438, width: 268, height: 114 }, { fontSize: 27, bold: true, color: C.ink });
