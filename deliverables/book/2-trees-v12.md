...
That is the whole claim in one picture: a frame of 120 members,
...
# Part 3: Variations and Future Systems

> The same frame can wear more than one roof, stand on more than one ground, and hang from nothing at all.

*The manufactured version of the method, and the two systems that turn it into something you can lift and hang: the shower-cap soft shell, the mast and the floor, and the floating dome.*
...
done. The cavity behind it holds 4 quilted layers and then
...
![The frame, three blanket quilts, and the shower cap over all of it.](../../deliverables/book/figures/hat-stack.png)
...
| 0 | $3,430 | $9,905 | -- |
| 3 | $3,637 | $9,905 | $6,269 |
| 7 | $3,914 | $10,510 | $6,596 |

At three blanket quilts the cap stack is 3,637 dollars against
9,905 for the hull and the bay sandwich it needs --
6,269 dollars less, which is the six thousand dollars the
campaign leads with. At seven quilts the saving is 6,596.

But look at the row the table does not show. The hull's cavity holds
4 layers and then its R-value stops moving. The cap has
no cavity. Its seventh quilt is a hat 15.6 per cent bigger than
...
lists at 18,834 dollars -- 68 dollars a square
foot of floor -- where the same dome with the hull lists at
27,581. The cap is not a discount shell. It is the standard
...
8 years and the membrane under it 12;
...
hull dome are the same 120 members, the same bays, the same
...
top. It is 12.0 feet of steel core clad in timber: metal where
...
240 dollar forging whose whole job is to be the point the
...
![The mast through the column, and the floor that clamps to it.](../../deliverables/book/figures/mast-floor.png)
...
The floor follows the same idea. Its hub is a 310 dollar steel
ring that clamps the mast; from the hub, 10 radial steel
spokes run to the base ring; timber decking covers the spokes, and a low
rail runs the edge. 277 square feet of floor that did not
...
| line | dollars |
|---|---|
| the mast through the column | $622 |
| the dome's own floor | $1,899 |
| the floating rig (next chapter) | $1,490 |
| **all three** | **$4,011** |

And the weight. The frame alone -- the 120 members of green
pine -- is 2,478 pounds. The shell or cap, the column, the
...
1,490 dollars of galvanized cable -- three legs of
28 feet -- a forged hanger that rides the ring, tree-hugger
...
trees -- the same 120 members, the same cap, the same
column, held from one point like a lamp.

![The same mast and floor, hung between trees on three cables.](../../deliverables/book/figures/floating-dome.png)
...
the weight of 2,478 pounds of frame before the floor and the
...
**`two_v_demo/book_tokens.py`** -- the 71-chapter book's
...
table prints the fold angles of 71 chapters' worth of
