...
def export_jig_cut_sequence(model: DomePhysicalModel, path: str | Path) -> None:
    """Export a practical pinwheel butt-cut sequence for the enforced-orientation jig.

    The uncut terminating member cannot occupy the same solid volume as the receiving wood.
    Therefore the cyan guide plate is treated as a RECEIVER-FACE PROXY during the cut: clamp
    the raw wedge in its red/green orientation fences, let the sacrificial tail pass the
    proxy plane, scribe/flush-cut, then dry-fit the actual receiving member in the same jig.
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "panel_id", "panel_type", "cut_step", "terminating_member",
            "receiving_member", "receiver_surface", "wedge_orientation",
            "sacrificial_butt_hangoff_in", "finished_blank_in", "jig_cut_blank_in",
            "fixture_action",
        ])
        for face in model.topology.faces:
            ...
            for step, member in enumerate(members, start=1):
                writer.writerow([
                    f"PANEL_{face.index + 1:03d}", face.face_type, step,
                    ...
                    "CLAMP_AXIS_AND_RED_GREEN_ROTATION; USE_CYAN_RECEIVER_PROXY; FLUSH_CUT_ORANGE_TAIL; DRY_FIT_RECEIVER",
                ])
...
def export_fabrication_package(model: DomePhysicalModel, directory: str | Path) -> None:
    ...
    for variant_number, (_, panel_indices) in enumerate(sorted(groups.items(), key=lambda item: item[1][0]), start=1):
        ...
        export_mesh_obj(jig["jig_hangoff"], variant_obj_dir / f"{variant_id}_SACRIFICIAL_BUTT_HANGOFF.obj", variant_id + "_SACRIFICIAL_BUTT_HANGOFF")

    with (directory / "JIG_VARIANTS.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["jig_variant", "wedge_orientation", "representative_panel", "compatible_panels", "panel_count", "pinwheel_receiver_surface", "jig_butt_hangoff_in"])
...
JIG_RED_FACE_FENCE_COLOR = (0.95, 0.10, 0.10, 0.42)
JIG_GREEN_FACE_FENCE_COLOR = (0.10, 0.92, 0.18, 0.42)
...
def add_jig_butt_hangoff(acc: MeshAccumulator, model: DomePhysicalModel, member: MemberInfo) -> None:
    ...
    origin = member.nominal_start + member.point_offset_in * member.inward
    # Determine the furthest point of the oblique finished butt cut along the member axis,
    # then place a square raw-stock end farther out by the requested sacrificial allowance.
    t_values: list[float] = []
    for (y, z), p in zip(local, butt_ring):
        generator_origin = origin + y * member.inward + z * member.face_normal
        t_values.append(float(np.dot(p - generator_origin, member.tangent)))
    stock_t = max(t_values) + hangoff
...
def build_fabrication_jig(model: DomePhysicalModel, panel_index: int) -> dict[str, MeshData | LineData]:
    ...
    # Copy the exact solved wood for the selected panel into the flat jig, then add a
    # separate translucent sacrificial BUTT HANG-OFF tail.  The tail is the raw stock that
    # deliberately runs past the solved receiver-face plane so the jig can enforce axis and
    # red/green sector orientation while the builder scribes / flush-cuts the compound butt.
    temp_wood = MeshAccumulator()
    temp_hangoff = MeshAccumulator()
    for member in members:
        add_raw_sector_member(temp_wood, model, member)
        add_jig_butt_hangoff(temp_hangoff, model, member)
    ...
    for member in members:
        ...
        for j in range(len(rpts)):
            lines.add(rpts[j], rpts[(j + 1) % len(rpts)], JIG_VERTEX_CUT_LINE_COLOR)

        # CYAN butt-cut guide = the actual receiving neighbor's PANEL_INTERIOR_FACE.
        # The orange translucent raw-stock tail deliberately crosses this plane.  In real
        # jig use this plane/fence is the flush-cut / scribe datum; the excess is discarded.
...
class JoinSolutionHud:
    ...
    def build(self, *, mode: str, edge_type: str, fold_deg: float, gap_deg: float) -> pygame.Surface:
        w, h = 420, 132
        ...
        left = pygame.Rect(8, 8, 196, 94)
        right = pygame.Rect(216, 8, 196, 94)
        ...
        remaining = max(0.0, 45.0 - fold_deg)
        text = f"{edge_type}-SEAM  fold {fold_deg:5.2f}°   included {180.0-fold_deg:6.2f}°   raw 45° - fold = {remaining:5.2f}°"
...
class DomeWorldApp:
    ...
    def _refresh_hud(self, force: bool = False) -> None:
        ...
        minimap_surface = self.minimap_hud.build(wedge_orientation=str(state[5]))
    ...
    def _upload_jig(self) -> None:
        ...
        for name in ("jig_wood", "jig_hangoff", "jig_fixture"):
...
def run_geometry_validation() -> None:
    ...
    print("      cyclic panel joints, 55 seams, both rigid seam strategies, and fabrication geometry.")
